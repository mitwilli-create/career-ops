...
export function loadAndRank(opts = {}) {
  ...
  // 2026-05-29 — CLI fallback. When phase-B-prime-daily (and similar
  // launchd-scheduled CLI invocations) run, the inline _CONTACTS_DATA
  // window-global may be absent or unparseable. Fall back to the
  // dashboard's pre-baked JSON at dashboard/data/contacts-annotated.json
  // — same source build-contacts-page.mjs writes. Closes 5+ consecutive
  // days of phase-B-prime-daily flapping (2026-05-25 → 2026-05-29) with
  // "No _CONTACTS_DATA found" FATAL on every scheduled run.
  if (contacts.length === 0) {
    ...
    if (existsSync(contactsJsonPath)) {
      try {
        ...
        // build-contacts-page.mjs writes { generated_at, contacts: [...], stats: ... }
        // but legacy contexts may write a bare array — handle both shapes so the
        // fallback survives a future schema flip in either direction.
  ...
  if (contacts.length === 0) {
    throw new Error('No contacts available — built dashboard/index.html missing _CONTACTS_DATA AND dashboard/data/contacts-annotated.json absent. Run `node scripts/build-dashboard.mjs` first.');
  }
