...
// Node imports are aliased with an underscore so the PURE extraction/matching
// functions above remain visibly fs-free; only getCvEmployerAllowlist (bottom
// of file) touches the filesystem.
...
// ─── Runtime allowlist loader (Qodo B8 refactor, 2026-07-06) ─────────────────
// Previously every consumer re-implemented "read cv.md, call parseCvEmployers,
// remember to fail closed" (rule 1671563) — build-apply-packs and
// claim-consistency each had their own copy, and a future caller could forget
// the fail-closed branch. This is the single supported entry point; the pure
// parseCvEmployers() stays exported for tests and custom sources.

/**
 * Load the employer allowlist from cv.md at runtime.
 *
 * FAIL-CLOSED CONTRACT: any problem (missing file, unreadable, no Experience
 * section, zero parsed employers) returns `{ ok: false, allowedEmployers:
 * empty Set, error }`. Callers must treat ok:false as a configuration failure
 * and HALT the gated operation — never proceed with an empty allowlist as if
 * "everything is allowed".
 *
 * @param {object} [opts]
 * @param {string} [opts.cvPath] — override for tests / non-standard layouts;
 *   defaults to <repo-root>/cv.md resolved from this file's location.
 * @returns {{ ok: boolean, allowedEmployers: Set<string>, cvPath: string, error: string|null }}
 */
export function getCvEmployerAllowlist(opts = {}) {
  const cvPath = opts.cvPath
    || _join(_dirname(_fileURLToPath(import.meta.url)), '..', 'cv.md');
  let text;
  try {
    text = _readFileSync(cvPath, 'utf-8');
  } catch (err) {
    return {
      ok: false,
      allowedEmployers: new Set(),
      cvPath,
      error: `cv.md unreadable at ${cvPath}: ${String(err?.message || err).slice(0, 120)}`,
    };
  }
  const allowedEmployers = parseCvEmployers(text);
  if (allowedEmployers.size === 0) {
    return {
      ok: false,
      allowedEmployers,
      cvPath,
      error: 'cv.md parsed but yielded ZERO employers (missing/renamed "## Experience" section?) — fail closed',
    };
  }
  return { ok: true, allowedEmployers, cvPath, error: null };
}
