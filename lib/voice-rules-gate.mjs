 ...
 *
 * Roll-up:
 *   FAIL  if any HARD-FAIL critical rule (10, 11, 12) fires
 *   WARN  if any soft rule scores < 0.6 OR overall weighted score 0.6–0.79
...
export function checkBannedVocab(text) {
  ...
  return {
    ...
    score: pass ? 1 : 0,
    matches,
    evidence: pass ? 'no banned-vocab matches' : `${matches.length} banned tokens: ${matches.slice(0, 3).map((m) => m.match).join(', ')}`,
  };
}
...
export function checkHedgeWords(text) {
  ...
  return {
    ...
    score: pass ? 1 : 0,
    matches,
    evidence: pass ? 'no hedge-words matches' : `${matches.length} hedges: ${matches.slice(0, 3).map((m) => m.match).join(', ')}`,
  };
}
...
export function checkSentenceLength(text) {
  ...
  const pass = max <= 35 && avg <= 24;
  // Score: linear penalty for avg > 22 or max > 35
  let score = 1;
  if (avg > 22) score -= Math.min(0.5, (avg - 22) * 0.05);
  if (max > 35) score -= Math.min(0.5, (max - 35) * 0.02);
  ...
  return {
    ...
    sentence_count: sentences.length,
    evidence: `avg ${Math.round(avg * 10) / 10} words/sentence, longest ${max} words; ${overCount} sentence(s) > 35-word cap`,
  };
}

export function checkRootDiscipline(text) {
  // Look for repeated 4+ char roots within a sliding 50-word window.
  ...
  for (let i = 0; i < words.length; i++) {
    const w = words[i];
    if (w.length < 5) continue; // skip short common words
  ...
  // Soft: 1 repeat → 0.7, 2 → 0.5, 3+ → 0.3
  const score = pass ? 1 : Math.max(0, 1 - repeats.length * 0.25);
  return {
    rule: 3,
...
export function runVoiceRulesGate(text) {
  ...
  for (const [id, result] of Object.entries(results)) {
    ...
    weightTotal += w;
    if (VOICE_RULES[id].critical && !result.pass) {
