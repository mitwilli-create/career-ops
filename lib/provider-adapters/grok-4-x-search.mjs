...
export async function refresh(cache, row, opts = {}) {
  ...
  if (!r.ok) {
    const txt = (await readText(r, ADAPTER_BODY_TEXT_TIMEOUT_MS, 'grok-adapter')).slice(0, 480);
    return {
      ok: false,
  ...
  const j = await readJson(r, ADAPTER_BODY_JSON_TIMEOUT_MS, 'grok-adapter');
  ...
  // 2026-07-08: every failure shape must carry explicit errors[] — an
  // ok:false with errors:undefined surfaces downstream as "WRITER FAILED: "
  // with an empty diagnostic (bug class:
  // findings-exit-code-conflated-with-spawn-failure).
  const errors = [];
  if (finishReason === 'length') {
    errors.push(`xAI output TRUNCATED at max_tokens=${body.max_tokens} (finish_reason=length, raw_content_length=${content.length}). Raise the cache's maxTokens in lib/refresh-cache-registry.mjs.`);
  } else if (!parsed) {
    errors.push(`xAI output not parseable as JSON (finish_reason=${finishReason || 'unknown'}, raw_content_length=${content.length}). Tail: "...${content.slice(-200).replace(/\s+/g, ' ')}"`);
  }
  if (regressionFlag) {
    errors.push(`grok-4-x-search regression: 0 citations returned (EPSILON 2026-05-19 known issue). Verifier should re-route to perplexity-agent-api or fall back to anthropic-sonnet.`);
  }

  return {
    ok: !!parsed && !regressionFlag && finishReason !== 'length',
    contentJson: parsed,
    costUsd,
    providerMetadata: {
...
function defaultPrompt(cache, row, schemaHint = null) {
  // 2026-07-08: same schema-pinning as the perplexity + anthropic adapters —
  // a schema-blind writer prompt guarantees verifier-rejected structure drift.
