 ...
 * @returns {Promise<{status,score,signals,gaps,model_used?,error?,rationale?,persona_signals_count?,unavailable?}>}
 *   `score` is null (with `unavailable: true`) on every error/unavailable
 *   path — a 0 would read as a confident "worst possible match" downstream.
 */
export async function hmMatchCheck({ slug, jdText = '', artifacts = {}, opts = {} } = {}) {
  if (!slug || typeof slug !== 'string') {
    return { status: 'error', error: 'slug (string) is required', score: null, signals: [], gaps: [], unavailable: true };
  }
