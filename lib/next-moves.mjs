...
function generateApplyMoves(apps, i, ctx) {
  ...
  for (const r of apps) {
    ...
    moves.push({
      ...
      evidence: _applyEvidence(r, t, lv, packReady[r.num], moves.length + 1),
      cost_hours,
...
// Phase A · A5 · HIGH-3 (2026-05-19) — accept a rank parameter so multiple
// sub-cards do not emit identical rationale. The rank pill in the card UI
// already conveys ordinal; the prose just confirms the throttle verdict.
function _applyEvidence(r, t, lv, packReady, rank) {
  const bits = [];
  if (/responded/i.test(r.status || '')) bits.push('They’ve responded — momentum');
  if (t.status === 'pickone') {
    bits.push(rank === 1 ? 'Throttle pick' : rank === 2 ? 'Backup 1' : 'Backup 2');
  } else if (t.status === 'defer') {
    bits.push('Defer — higher-scored sibling open');
  }
  if (lv.status === 'active')             bits.push('alive overnight');
  else if (lv.status === 'expired_needs_review') bits.push('liveness uncertain — verify first');
  bits.push(packReady ? 'pack ready' : 'pack needs build (~2 extra hrs)');
  return bits.join(' · ');
}
