 ...
 *   Pangram        POST https://text.api.pangram.com/v3       (v3 endpoint, x-api-key auth; v1/v2 vendor-deprecated)
 *
...
async function callGPTZero(text) {
  // G38 (2026-05-24): per-detector + global disable flags. Preserves API key
  // for future re-enabling. When all 3 saturate (current vendor state per
  // data/detector-recalibration-audit-2026-05-23.md), set _DISABLED=1 to stop
  // paying for API calls that the L1 gate explicitly ignores under
  // UNCALIBRATED signal. Re-enable individually when vendor improvement ships.
  if (process.env.AI_DETECTION_DISABLED === '1' || process.env.GPTZERO_DISABLED === '1') {
    return { skipped: true, reason: 'GPTZERO_DISABLED via env flag (G38 vendor-saturation pause)', prob: null };
  }
...
async function callOriginalityAI(text) {
  // G38 (2026-05-24): per-detector + global disable flags. See callGPTZero comment above.
  if (process.env.AI_DETECTION_DISABLED === '1' || process.env.ORIGINALITY_DISABLED === '1') {
    return { skipped: true, reason: 'ORIGINALITY_DISABLED via env flag (G38 vendor-saturation pause)', prob: null };
  }
...
async function callPangram(text) {
  // G38 (2026-05-24): per-detector + global disable flags. See callGPTZero comment above.
  if (process.env.AI_DETECTION_DISABLED === '1' || process.env.PANGRAM_DISABLED === '1') {
    return { skipped: true, reason: 'PANGRAM_DISABLED via env flag (G38 vendor-saturation pause)', prob: null };
  }
...
function loadCalibratedThresholds() {
  ...
  try {
    ...
    // GAP-RES-06 (2026-05-24) — skip the band-shape validator for detectors
    // explicitly marked `disabled: true`; their threshold entry intentionally
    // omits CLEAR / CRIT bands. signalQuality() returns 'DISABLED' for them.
...
function assignBand(prob, detectorName) {
  ...
  if (!t) return null;
  // GAP-RES-06 (2026-05-24) — disabled detectors have no band structure;
  // defensive null-return prevents reading t.CRIT.min on a `disabled: true`
  // entry. In practice prob is null when a detector is disabled (the
  // detector function returns skipped:true), so the prob==null guard above
  // already catches this — but the explicit check below guards against
  // hand-edited current-thresholds.json that pairs a disabled marker with
  // a non-null prob (e.g. cached result from before the disable flipped).
  if (t.disabled === true) return null;
...
function signalQuality(detectorName) {
  ...
  // GAP-RES-06 (2026-05-24) — return 'DISABLED' when the threshold entry
  // marks the detector as not-called. Before the fix, a disabled detector's
  // bands defaulted to phantom CLEAR.max=0 / CRIT.min=1 (gap=1.0) from null
  // baseline data, and this function returned 'GOOD' incorrectly — making
  // anyGood=true downstream + gateBlocks=true even when no real detector
  // had actionable signal. The 'DISABLED' state propagates honestly.
  if (t[detectorName]?.disabled === true) return 'DISABLED';
...
function buildResult({ gptzero, originality, pangram, fromCache = false, ackDetectionDegraded = false }) {
  ...
  if (gateBlocks) {
    ...
    !anyGood
    && (gzSignalQuality      === 'USELESS' || gzSignalQuality      === 'DISABLED')
    && (origSignalQuality    === 'USELESS' || origSignalQuality    === 'DISABLED')
    && (pangramSignalQuality === 'USELESS' || pangramSignalQuality === 'DISABLED')
  ) {
    // AAA-4 (fail-secure default): when NO detector has actionable signal
    // (all USELESS or DISABLED), the gate has no ground truth. Mark
    // passes=null ("unchecked"), set degraded=true, and require caller to
    // opt in via opts.ackDetectionDegraded=true.
    // GAP-RES-06 (2026-05-24) — extended to accept DISABLED alongside
    // USELESS. Previously a configuration of (USELESS, USELESS, USELESS)
    // was required; now (USELESS, DISABLED, DISABLED) — the actual G38
    // state — also fail-secures correctly.
    degraded = true;
