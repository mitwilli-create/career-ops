...
export const PAGE_REGISTRY = [
  ...
  // 2026-05-20 — sectionAnchor added so the home sidebar uses in-page #anchor
  // hrefs (preserves scroll-spy IntersectionObserver via data-section attr)
  // while non-home pages link to /#anchor.
  { id: 'overview',     href: '/#overview-section',         sectionAnchor: 'overview-section',         label: 'Overview',         icon: '📊', group: 'dashboard' },
  { id: 'apply-now',    href: '/#apply-now-section',        sectionAnchor: 'apply-now-section',        label: 'Apply-Now',        icon: '🎯', group: 'dashboard' },
  { id: 'all-evals',    href: '/#all-evaluations-section',  sectionAnchor: 'all-evaluations-section',  label: 'All Evaluations',  icon: '📋', group: 'dashboard' },
  // GAP-RES-23 (2026-05-24) — META-AUDIT v3 follow-up: Pipeline Pending +
  // Batch Runs now have anchored in-page summary sections in addition to
  // the modal triggers in the Actions group below. The Actions-group entries
  // (id:'pending', id:'batch-runs') open the full modal flow; the
  // Dashboard-group entries here scroll to compact summary panels in
  // scripts/build-dashboard.mjs that link back into the modals.
  { id: 'pipeline-pending', href: '/#pipeline-pending-section', sectionAnchor: 'pipeline-pending-section', label: 'Pipeline Pending', icon: '⏳', group: 'dashboard' },
  { id: 'batch-runs-anchor', href: '/#batch-runs-section',     sectionAnchor: 'batch-runs-section',     label: 'Batch Runs',       icon: '📦', group: 'dashboard' },
  { id: 'trends',       href: '/#trends-panel',             sectionAnchor: 'trends-panel',             label: 'Trends',           icon: '📈', group: 'dashboard' },
  { id: 'companies',    href: '/#companies-panel',          sectionAnchor: 'companies-panel',          label: 'Companies',        icon: '🏢', group: 'dashboard' },
  // Group 2: Pages (cross-page links — aria-current="page" when active)
  { id: 'home',         href: '/',                          label: 'Dashboard',        icon: '⚡', group: 'pages' },
  // P0.8 (2026-05-20) — collapsed Network + Network DB into a single sidebar
  // entry per researcher-converged-with-dealbreaker recommendation. Cards
  // stays reachable at /contacts.html as a bookmarkable deep-link surface;
  // build-contacts-page.mjs sets pageId='network' so its sidebar also
  // highlights this same entry. In-page Table|Cards segmented controls on
  // both pages communicate sub-surface state via aria-pressed (NOT aria-
  // current — that stays on the sidebar). See data/p08-toolbar-convergence-
  // research-2026-05-20.md.
  { id: 'network',      href: '/network-database.html',     label: 'Network',          icon: '👥', group: 'pages' },
  // Group 3: Actions (modal triggers)
  // On home, fires the in-page modal function. On non-home, navigates to /?open=<id>
  // Blind-review #18 (2026-07-07) — labels renamed to verb phrases so they no
  // longer duplicate the Dashboard-group nav anchors verbatim ("Batch Runs"
  // and "Pipeline" each appeared twice with identical styling and unknowable
  // difference). Nav anchors scroll to in-page summary panels; these open the
  // full modal flows — the ellipsis marks them as dialog openers. IDs are
  // load-bearing (the /?open=<id> cross-page flow keys off them) — unchanged.
  { id: 'pending',      action: 'modal:pending',            label: 'Open pipeline flow…',   icon: '⏳', group: 'actions' },
  { id: 'batch-runs',   action: 'modal:batch-runs',         label: 'Open batch status…',    icon: '📦', group: 'actions' },
  { id: 'industries',   action: 'drill:industry-gap',       label: 'Review industry gaps…', icon: '🎯', group: 'actions' },
  { id: 'settings',     action: 'modal:settings',           label: 'Open settings…',        icon: '⚙️', group: 'actions' },
];
...
export function getDashboardSidebar({ currentPage = 'home' } = {}) {
  ...
  const renderItem = (p) => {
    ...
    if (p.href) {
      const sublabel = p.sublabel
        ...
        : '';
      // On HOME, dashboard-group items use in-page #anchor hrefs (not /#anchor)
      // so the browser scrolls instead of reloading; data-section preserves
      // IntersectionObserver scroll-spy. Cross-page links keep their canonical
      // hrefs.
      let resolvedHref = p.href;
      let dataSectionAttr = '';
      if (isHome && p.group === 'dashboard' && p.sectionAnchor) {
        resolvedHref = `#${p.sectionAnchor}`;
        dataSectionAttr = ` data-section="${esc(p.sectionAnchor)}"`;
      }
      return `<a href="${esc(resolvedHref)}" class="sidebar-link${activeClass}" title="${esc(p.label)}"${ariaCurrent}${dataSectionAttr}>
    ...
    if (isHome) {
      ...
      let onclick = '';
      if (actionType === 'modal') {
        // Map to existing handlers in build-dashboard.mjs. For 'pending', prefer
        // the 5-stage Pipeline flow modal (shipped PR #92) and fall back to the
        // legacy stat panel only when _openPipelineFlowModal isn't available
        // (e.g. older cached page). Bare toggleStatPanel('pending') is the
        // pre-PR #92 affordance and was what Mitchell observed mid-2026-05-22
        // when he reported the sidebar Pipeline link "doesn't fire" the modal.
        if (actionTarget === 'pending') onclick = "(window._openPipelineFlowModal||function(){toggleStatPanel('pending');})();closeSidebar();";
  ...
  return `<aside class="sidebar" id="sidebar" aria-label="Primary navigation">
    <a class="sidebar-brand" href="/" aria-label="Career-Ops home">
      ...
      <span class="sidebar-brand-group">
        <span class="sidebar-brand-name">Career-Ops</span>
        <span class="sidebar-brand-byline">Mitchell Williams</span>
      </span>
    </a>
...
/**
 * Render just the BRAND + NAV portion of the sidebar (without the wrapping
 * <aside>). Used by build-dashboard.mjs where the <aside> is hand-written
 * and contains additional widgets (update drawer, batch widget, pipeline
 * buttons, freshness chip) below the <nav>.
 *
 * 2026-05-20 — added for BRAVO followup Item 2 / AAA-2 (consistent grouping
 * on home).
 */
export function getDashboardSidebarInner({ currentPage = 'home' } = {}) {
  const fullAside = getDashboardSidebar({ currentPage });
  // Strip wrapping <aside ...> ... </aside> — keep contents.
  return fullAside
    .replace(/^[\s]*<aside[^>]*>/, '')
    .replace(/<\/aside>[\s]*$/, '')
    .trim();
...
export function getDashboardShellCSS({ scopeForStandalonePage = false } = {}) {
  ...
  // we emit the layout/typography aliases (--sidebar-w, --font-sans, --radius-*)
  // that aren't yet in tokens.css per ARCH.41 §0 deferral list, plus base styles.
  //
  // ARCH.41 (2026-05-21) — color tokens (`--bg`, `--surface`, `--text*`, `--green-*`,
  // `--blue-*`, `--amber-*`, `--red-*`, `--border*`, `--focus-ring`) are now sourced
  // from `dashboard/css/tokens.css`, which `renderDashboardShell` injects via
  // `<link rel="stylesheet">` before this inline <style>. tokens.css ships dark
  // defaults at `:root` (matches the early-paint script's dark default) +
  // `:root[data-theme="light"]` + `@media (prefers-color-scheme: light)
  // :root:not([data-theme="dark"])` for match-OS. Edit `tokens/master.json`
  // and re-run `node scripts/transform-tokens.mjs` to change a color value.
  //
  // When `scopeForStandalonePage: false` (build-dashboard.mjs main SPA), no
  // token block emitted — that pipeline has its own inline tokens at
  // `scripts/build-dashboard.mjs` :root blocks per ARCH.41 §0 deferral.
...
.sidebar-nav-group-label {
  ...
  /* BRAVO followup 2026-05-20 Item 9 / AA-10 — text-3 (#64748b) passes on
     pure white (4.66:1) but fails on --surface-2 (#f0f2f5, 4.24:1) where
     the sidebar sits in some states. Bumped to --text-2 (#334155) which
     passes both: 11.7:1 on white, 11.0:1 on surface-2. */
  color: var(--text-2);
...
.sidebar-sublabel {
  font-size: 10px;
  font-weight: 400;
  /* BRAVO followup 2026-05-20 Item 9 / AA-10 — see comment above on
     sidebar-nav-group-label; same surface-2 issue. */
  color: var(--text-2);
  margin-top: 1px;
}
...
<head>
  ...
  <!-- B1.3 (2026-05-20) — set data-theme BEFORE any stylesheet so the
       saved palette renders on first paint (avoid FOUC). Default = dark. -->
  <script>(function(){try{var v=localStorage.getItem('career-ops-theme');if(['dark','light','match-os'].indexOf(v)===-1)v='dark';document.documentElement.setAttribute('data-theme',v);}catch(e){document.documentElement.setAttribute('data-theme','dark');}})();</script>
  <!-- ARCH.41 (2026-05-21) — canonical color/spacing/radius tokens. Edit
       tokens/master.json + re-run node scripts/transform-tokens.mjs. -->
  <link rel="stylesheet" href="/dashboard/css/tokens.css">
