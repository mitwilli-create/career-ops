 ...
 * @param {string} company
 * @param {number} timeoutMs
 * @returns {Promise<{ canonical: string|null, evidenceUrl: string, score: number, atsHost: string }>}
 */
async function findCanonicalByTitleCompany(context, title, company, timeoutMs) {
  if (!title || !company) {
    return { canonical: null, evidenceUrl: '', score: 0, atsHost: '' };
  }
 ...
 *             confidence: number, original: string, error?: string,
 *             title?: string, company?: string, matchScore?: number }} CanonicalizeResult
 */
...
if (isCLI) {
  ...
  const results = await canonicalizeBatch(urls, {
    ...
    onProgress: ({ i, total, url, result }) => {
      ...
      const head = `${icon} [${i}/${total}] ${result.source.padEnd(24)} (conf=${result.confidence.toFixed(2)})`;
      console.log(head);
      console.log(`    in:  ${url}`);
