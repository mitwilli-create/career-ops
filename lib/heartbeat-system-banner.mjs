 ...
 *
 * Banner confirming which Tier 5 system features are active, plus a few
 * small adjacent banners (CDP auth health, polish summary, cron health,
 * discard-pattern of the week). The runway-alert section that used to live
 * here was retired 2026-05-25 alongside the broader runway-coupling sweep;
 * only the helpers below remain.
 *
 ...
 *   - Council orchestration layer present? (lib/council.mjs exists)
 *
...
function getActiveFeatures() {
  // Phase A · A6 · MEDIUM-1 (2026-05-19) — Burst mode row is OMITTED when
  // MONTHLY_BUDGET_USD_BURST is unset or zero. The placeholder "Set X to
  // enable" row was noise on every normal day.
  ...
  const features = [
    ...
    { label: 'Time-to-Offer estimator',          active: exists('lib/tto-estimator.mjs'),
      detail: 'Per-company velocity hint — fast / med / slow / glacial' },
    { label: 'Company-toxicity flagging',        active: exists('lib/toxicity-scorer.mjs'),
      detail: 'Flag-for-review composite (never auto-trash)' },
    { label: 'Recruiter pipeline-density signal', active: exists('dashboard-server.mjs'),
      detail: 'Warm-network coverage of active apply-now roles' },
  ...
  if (burstActive) {
    features.push({
      label: 'Burst mode',
      active: true,
      detail: `+$${process.env.MONTHLY_BUDGET_USD_BURST} until ${process.env.MONTHLY_BUDGET_BURST_UNTIL || 'untouched until set'}`,
    });
  }
  return features;
}
 ...
 */
export function renderSystemBanner({ format = 'html', dashboardUrl = '' } = {}) {
  ...
  // Phase A · A3 · HIGH-1 (2026-05-19) — 7-row table replaced with a one-line
  // muted footer. Detailed feature breakdown lives on the dashboard; the
  // morning email at 09:01 PT only needs the activeCount/total ratio.
  const detailsLink = dashboardUrl
    ? ` · <a href="${dashboardUrl}/?focus=system-status" style="color:#16a34a;text-decoration:none">details →</a>`
    : '';
  return `<div style="font-size:12px;color:#6b7280;margin:8px 0;font-family:-apple-system,BlinkMacSystemFont,sans-serif">✓ ${activeCount}/${features.length} Tier 5 features active${detailsLink}</div>`;
}

// `renderRunwayAlert({ pipelineDensity, format })` was retired 2026-05-25
// alongside the broader runway-coupling sweep. The function used to render
// a banner sized off `pipelineDensity.runway_weeks` + `pipelineDensity.runway_alert`,
// both of which were removed from `dashboard-server.mjs::computeRecruiterPipelineDensity`
// in the same sweep. No replacement banner is shipped — the email surface
// no longer leads with a runway-framed urgency signal.
