...
/**
 * Canonicalize a path, resolving symlinks in every component that exists.
 *
 * `path.resolve`/`normalize` collapse `..` textually but do NOT follow symlinks,
 * so a symlink living inside an allowed directory could otherwise redirect a
 * read to .env / credentials outside the allowlist and still pass a string
 * prefix check. We realpath the longest existing prefix (defeating symlink
 * traversal) and re-append any not-yet-existing tail (which cannot be a symlink
 * because it does not exist) — so existsSync/access on missing paths still work.
 *
 * @param {string} p
 * @returns {string} canonical absolute path
 */
function canonicalize(p) {
  let abs = resolve(normalize(String(p)));
  const tail = [];
  for (;;) {
    try {
      ...
      if (parent === abs) return resolve(abs, ...tail); // reached fs root; nothing below exists
...
export function createReadonlyFS(allowedPaths = []) {
  ...
  // Canonicalize all allowed paths once at construction time (resolves symlinks
  // so the prefix check in guard() compares real path against real path).
  const resolved = allowedPaths.map(p => canonicalize(p));
...
export function createWriteableFS(writableSubdirs = []) {
  ...
  const resolved = writableSubdirs.map(p => canonicalize(p));
