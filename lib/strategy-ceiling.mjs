...
<div class="strategy-card" style="font-family:system-ui;max-width:420px;padding:14px 16px;border-radius:8px;border:1px solid #e5e7eb;background:#fff;box-shadow:0 2px 8px rgba(0,0,0,.08);">
  ${_degraded ? `<div style="font-size:11px;color:#f59e0b;margin-bottom:8px;background:#fef3c7;padding:6px 8px;border-radius:4px;">⚠ Strategy ceiling unavailable — LLM did not respond. No fabricated numbers shown.${retryButton}${retryExhaustedNote}</div>` : ''}
...
function renderStrategyCardDataFirst(result) {
  ...
  <li style="margin-bottom:10px;padding:8px 10px;background:#f8fafc;border-left:3px solid #16a34a;border-radius:4px;">
    <div style="font-size:12px;font-weight:600;color:#111;margin-bottom:2px;">
      ${_escHtml(e.what_data)}${confBadge(e.confidence)}
    </div>
    <p style="margin:0 0 4px;font-size:12px;color:#374151;line-height:1.45;">${_escHtml(e.what_it_reveals)}</p>
  ...
  const gapItems = (open_gaps || []).map((g) => {
    ...
    return `
  <li style="margin-bottom:10px;padding:8px 10px;background:#fef3c7;border-left:3px solid #f59e0b;border-radius:4px;">
    <div style="font-size:12px;font-weight:600;color:#111;margin-bottom:2px;">
      ${_escHtml(g.what_is_missing)}
    </div>
    <p style="margin:0 0 6px;font-size:12px;color:#374151;line-height:1.45;">${_escHtml(g.why_it_matters)}</p>
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
      <button type="button" data-strategy-fill-gap-agent="${agent}" style="padding:4px 10px;font-size:11px;background:#f59e0b;border:1px solid #d97706;color:#fff;border-radius:4px;cursor:pointer;font-weight:600;">Queue ${agent}</button>
      ${cost ? `<span style="font-size:10px;color:#6b7280;">est ${cost}</span>` : ''}
    </div>
  </li>`;
  }).join('');

  const noContentBlock = (evidence.length === 0 && open_gaps.length === 0)
    ? `<p style="margin:8px 0;padding:8px 10px;background:#fee2e2;border-left:3px solid #ef4444;border-radius:4px;font-size:12px;color:#7f1d1d;">No evidence cited and no gaps flagged. This is likely a degraded LLM response — re-run the popout to retry.</p>`
    : '';

  return `
<div class="strategy-card strategy-card--data-first" data-popout-mode="data-first" style="font-family:system-ui;max-width:480px;padding:14px 16px;border-radius:8px;border:1px solid #e5e7eb;background:#fff;box-shadow:0 2px 8px rgba(0,0,0,.08);">
  ...
  <div style="margin-bottom:12px;">
    ...
    ${reasoning ? `<p style="margin:6px 0 0;font-size:11px;color:#6b7280;font-style:italic;">${_escHtml(reasoning)}</p>` : ''}
  </div>
  ` : ''}

  ${evidence.length > 0 ? `
  <!-- Evidence on disk -->
  <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#374151;margin:8px 0 6px;">Evidence on disk</div>
  ...
  ${open_gaps.length > 0 ? `
  <!-- Open gaps -->
  <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#374151;margin:8px 0 6px;">Open gaps · honest read</div>
  <ul style="list-style:none;padding:0;margin:0;">
    ${gapItems}
  </ul>
  ` : ''}
