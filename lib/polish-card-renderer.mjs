...
// Dark-first tints. bg uses CSS var so it adapts to light mode via --surface.
// border/fg keep semantic color; light-mode overrides live in polishCardStyles().
const VERDICT_TO_TINT = {
  APPROVED:    { bg: 'var(--surface,#0d1f16)', border: '#16a34a', fg: '#4ade80' },
  NEEDS_HUMAN: { bg: 'var(--surface,#1e1610)', border: '#d97706', fg: '#fbbf24' },
  REJECTED:    { bg: 'var(--surface,#1f0d0d)', border: '#dc2626', fg: '#f87171' },
  ABANDONED:   { bg: 'var(--surface,#0e0f1e)', border: '#6366f1', fg: '#a5b4fc' },
  NEVER:       { bg: 'var(--surface,#11131c)', border: 'var(--border,#232737)', fg: 'var(--text-4,#9a9aa6)' },
};
...
export function renderPolishDcard(status, opts = {}) {
  ...
  if (!status || !status.verdict) {
    // Never-polished state — headline + primary button visible; explainer
    // collapsed behind <details>. Audit Tier 1 #2 (2026-05-20) added the
    // narrative-grade text for new users; 2026-05-24 demoted it behind a
    // disclosure so the action button gets prominence (matches P06 hybrid
    // layout pattern in data/p06-popout-actions-ux-research-2026-05-20.md).
    return `
<div class="dcard dcard--polish dcard--polish-never" style="border-color:${VERDICT_TO_TINT.NEVER.border};background:${VERDICT_TO_TINT.NEVER.bg};margin-bottom:10px">
  <div class="dcard-label" style="color:${VERDICT_TO_TINT.NEVER.fg}">Apply-pack polish</div>
  <div style="font-size:13px;color:#374151;margin-top:4px;line-height:1.55">
    <span style="font-size:16px;vertical-align:middle">⚪</span> &nbsp; <strong style="color:var(--text-2,#e4e4e7)">Polish not yet run.</strong> <span style="color:var(--text-3,#b8b8c0)">Recommended before you submit.</span>
    ${packSlug ? `<div style="margin-top:8px"><button type="button" class="polish-action-btn" onclick="event.stopPropagation();runPolishNow('${esc(rowId)}','${esc(packSlug)}')">Run polish now &rarr;</button></div>` : ''}
    <details class="polish-explainer" style="margin-top:10px">
      <summary style="cursor:pointer;font-size:11.5px;color:var(--text-4,#9a9aa6)">What is polish?</summary>
      <div style="margin-top:6px;font-size:12px;color:var(--text-4,#9a9aa6);line-height:1.55">
        A 4-round critic &rarr; author &rarr; adjudicator &rarr; adversarial loop over your CV, cover letter, form answers, impact doc, references, and referrals. Converges when every artifact clears a 0.99 confidence bar. Usually $15&ndash;$30 and 20&ndash;40 minutes per pack.
      </div>
    </details>
  </div>
</div>`.trim();
  ...
  const artifactRows = Object.entries(artifacts).map(([name, a]) => {
    ...
    return `<tr>
      <td style="padding:3px 8px 3px 0;font-size:12px;color:var(--text-2,#e4e4e7);font-weight:600">${esc(name)}</td>
      <td style="padding:3px 8px 3px 0;font-size:12px;color:var(--text-3,#b8b8c0)">conf ${conf}</td>
      <td style="padding:3px 8px 3px 0;font-size:12px;color:var(--text-3,#b8b8c0)">${a.rounds_used || 0} round${a.rounds_used === 1 ? '' : 's'}</td>
  ...
  const blockerList = blockingIssues.length
    ? `<details style="margin-top:8px">
         ...
         <ul style="margin:6px 0 0;padding-left:20px;font-size:12px;color:var(--text-2,#e4e4e7)">
  ...
  const actions = (isActionable && packSlug) ? `
    <div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:10px">
      ...
      <button type="button" class="polish-action-btn polish-action-btn--skip" onclick="event.stopPropagation();skipRowFromPolish('${esc(rowId)}','${esc(packSlug)}')" title="Discard this role — polish couldn't save it and JD-fit isn't there">Discard Role</button>
    </div>` : '';
...
export function polishCardStyles() {
  return `
/* Polish status — badge + drawer card (updated 2026-05-26: dark-first) */
.polish-badge {
...
/* Dark-first button defaults (dashboard is dark by default) */
...
.dcard--polish .polish-action-btn:hover {
  background: rgba(255,255,255,0.12);
  border-color: var(--border-strong, #3a3f4a);
}
.dcard--polish .polish-action-btn:active { transform: translateY(1px); }
.dcard--polish .polish-action-btn--rerun { font-weight: 600; }

/* Light-mode overrides — restore original tinted backgrounds */
body:not(.dark) .dcard--polish.dcard--polish-approved  { background: #dcfce7 !important; border-color: #86efac !important; }
body:not(.dark) .dcard--polish.dcard--polish-needs_human { background: #fef3c7 !important; border-color: #fcd34d !important; }
body:not(.dark) .dcard--polish.dcard--polish-rejected  { background: #fee2e2 !important; border-color: #fca5a5 !important; }
body:not(.dark) .dcard--polish.dcard--polish-abandoned { background: #e0e7ff !important; border-color: #a5b4fc !important; }
body:not(.dark) .dcard--polish.dcard--polish-never     { background: #f9fafb !important; border-color: #e5e7eb !important; }
body:not(.dark) .dcard--polish .polish-action-btn {
  background: rgba(255,255,255,0.5) !important;
  color: #374151 !important;
  border-color: #d1d5db !important;
}
body:not(.dark) .dcard--polish .polish-action-btn:hover {
