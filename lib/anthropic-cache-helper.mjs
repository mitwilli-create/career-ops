 ...
 *   - modes/_profile.md
 *
 * Cache lifetime: 1 hour (explicit ttl:'1h' on all cache_control blocks).
 * Anthropic changed the default from 1h → 5min on 2026-03-06; we now opt
 * back into 1h explicitly so batch-style refresh-master runs hit the cache
 * across the full batch window (which often exceeds 5 min).
 *
 * NOTE on the repo's 3-day cache-TTL convention (Qodo rule 1670738, reviewed
 * 2026-07-06): it CANNOT apply here. This ttl is Anthropic's server-side
 * prompt-cache parameter and the API accepts exactly two values, '5m' and
 * '1h' — there is no multi-day option. '1h' is already the maximum. The
 * 3-day convention governs LOCAL caches (e.g. triage.mjs URL cache, now 3d).
 *
 * Pricing note: 1h cache_write tokens cost 2× base input (vs 1.25× for
 * 5-min). Cache_read tokens remain 0.1× base. Net savings still strongly
 * positive for corpus sizes ≥ 5k tokens + >2 reads per write in a batch.
 *
 * Verified API behavior (WebFetch of docs.anthropic.com 2026-05-19):
 *   - cache_control: { type: 'ephemeral', ttl: '1h' } on a content block creates a
...
const CACHE_BODY_JSON_TIMEOUT_MS = 90_000;
const CACHE_BODY_TEXT_TIMEOUT_MS = 30_000;
...
export async function callAnthropicCached(req) {
  ...
  if (!r.ok) {
    const txt = (await readText(r, CACHE_BODY_TEXT_TIMEOUT_MS, 'cache-helper')).slice(0, 480);
  ...
  const j = await readJson(r, CACHE_BODY_JSON_TIMEOUT_MS, 'cache-helper');
  ...
  // Cost per 1M tokens. Cache-write reflects 1h TTL (2× base input per Anthropic pricing
  // updated 2026-03-06). 5-min write was 1.25× base. Cache-read is 0.1× base for both TTLs.
  const PRICING = {
    'claude-sonnet-4-6': { input: 3.0, output: 15.0, cacheWrite: 6.0,   cacheRead: 0.30 },
    'claude-opus-4-7':   { input: 15.0, output: 75.0, cacheWrite: 30.0,  cacheRead: 1.50 },
    'claude-haiku-4-5':  { input: 0.25, output: 1.25, cacheWrite: 0.50,  cacheRead: 0.025 },
  };
  ...
  return {
    ...
    modelUsed: model,
    costUsd,
    // 2026-07-08: additive — lets adapters detect max_tokens truncation
    // instead of silently parsing a cut-off JSON body.
