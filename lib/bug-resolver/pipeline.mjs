...
// Parse a source_surface string into { path, lineNum }. Strips the `:NNN`
// line-number suffix that the v2 bridge (regression-guard intake) appends.
// 2026-05-25: pre-fix, "dashboard-server.mjs:9223" was looked up as a literal
// filename (because join(REPO_ROOT, "dashboard-server.mjs:9223") doesn't exist
// on disk) — that bug caused bug-2026-05-25-034 to fail audit with
// "Source ... unavailable (missing)". The fix splits file from line.
function splitSourceSurface(sourceSurface) {
  // Match trailing :NNN where NNN is digits (so we don't break Windows-style
  // drive prefixes like C: which won't appear in this codebase but is a
  // common foot-gun).
  const m = String(sourceSurface || '').match(/^(.+?):(\d+)$/);
  if (m) return { path: m[1], lineNum: Number(m[2]) };
  return { path: String(sourceSurface || ''), lineNum: null };
}

// Read ±contextLines around the cited line. Returns array of {lineNum, text}.
// Used for content-fingerprint validation when the file has drifted.
function readLineContext(absPath, lineNum, contextLines = 5) {
  try {
    ...
    for (let i = start; i < end; i++) {
      out.push({ lineNum: i + 1, text: lines[i] });
    }
    return out;
  } catch { return []; }
}

// Re-resolve a cited file:line by grep'ing for fingerprint context.
// Returns the new lineNum if found, or null if unrecoverable.
// 2026-05-25 — content-fingerprint re-resolution per /feedback chain.
function reResolveByFingerprint(absPath, lineContext) {
  if (!existsSync(absPath) || !Array.isArray(lineContext) || lineContext.length < 1) return null;
  try {
    const fileLines = readFileSync(absPath, 'utf-8').split('\n');
    // The center entry of the original line_context is the cited line.
    // Use it + 1 neighbour as a 2-line fingerprint (more robust than a single
    // line, less brittle than the full ±5 context which might span function
    // boundaries that get re-indented during refactors).
    const center = Math.floor(lineContext.length / 2);
    const fp1 = lineContext[center]?.text?.trim();
    const fp2 = lineContext[center + 1]?.text?.trim() || lineContext[center - 1]?.text?.trim();
    if (!fp1 || fp1.length < 8) return null; // skip pathologically short fingerprints
    for (let i = 0; i < fileLines.length; i++) {
      if (fileLines[i].trim() === fp1) {
        // If we have a 2-line fingerprint, require the next/prev line to match too
        if (fp2) {
          if (fileLines[i + 1]?.trim() === fp2 || fileLines[i - 1]?.trim() === fp2) {
            return i + 1;
          }
        } else {
          return i + 1;
...
// Returns { content, hash, drifted, exists, lineNum, lineDrifted, resolvedLineNum }.
function readSource(sourceSurface, expectedHash, lineContextFromIntake = null) {
  ...
  // Split path from :LINE suffix (2026-05-25 fix — see splitSourceSurface above)
  const { path: filePath, lineNum } = splitSourceSurface(sourceSurface);
  const absPath = filePath.startsWith('/')
    ? filePath
    : join(REPO_ROOT, filePath);
  if (!existsSync(absPath)) {
    return { content: '', hash: null, drifted: false, exists: false, kind: 'missing', lineNum };
  }
  try {
    const content = readFileSync(absPath, 'utf-8');
    const hash = hashCite(content);
    // Content-fingerprint re-resolution (2026-05-25 /feedback chain). If
    // intake captured line_context and the file has drifted, try to relocate
    // the cited line.
    let lineDrifted = false;
    let resolvedLineNum = lineNum;
    if (lineNum && Array.isArray(lineContextFromIntake) && lineContextFromIntake.length > 0) {
      const currentLines = content.split('\n');
      const currentAtLine = (currentLines[lineNum - 1] || '').trim();
      const centerIdx = Math.floor(lineContextFromIntake.length / 2);
      const originalAtLine = (lineContextFromIntake[centerIdx]?.text || '').trim();
      if (originalAtLine && originalAtLine !== currentAtLine) {
        lineDrifted = true;
        const newLineNum = reResolveByFingerprint(absPath, lineContextFromIntake);
        if (newLineNum) resolvedLineNum = newLineNum;
      }
    }
    return {
      content,
    ...
    return { content: '', hash: null, drifted: false, exists: false, kind: 'unreadable', error: err.message, lineNum };
  }
}
...
export async function processBug(bug, opts = {}) {
  ...
  // Re-read source fresh. Pass intake_metadata.line_context if present so
  // readSource can re-resolve the cited :LINE via content fingerprint when
  // the file has drifted since intake (2026-05-25 /feedback chain).
  const lineContextFromIntake = bug.intake_metadata?.line_context || null;
  const sourceInfo = readSource(bug.source_surface, bug.source_hash, lineContextFromIntake);
  ...
  if (sourceInfo.lineDrifted) {
    if (sourceInfo.resolvedLineNum && sourceInfo.resolvedLineNum !== sourceInfo.lineNum) {
      human_reasons.push(`Source line drifted ${sourceInfo.lineNum} → ${sourceInfo.resolvedLineNum} (re-resolved via content fingerprint)`);
    } else {
      human_reasons.push(`Source line ${sourceInfo.lineNum} drifted beyond fingerprint recovery — cited code no longer findable`);
    }
  }
  ...
  // ─── Phase 6: Verify ───────────────────────────────────────────────────
  // Wave 2 task 7 (2026-05-28): augmented with Margaret Hamilton persona
  // fan-in. Flash provides the structural verdict; Hamilton independently
  // scans the same prompt for the catalog's "writer claims a property
  // the consumer can't trust" bug family (state-write-without-disk-write,
  // sentinel-truthy-gating, polish-no-timeout, write-without-rebuild-
  // propagation-gap). If Hamilton flags HIGH and Flash returns PASS, the
  // result escalates to NEEDS_HUMAN (deferring to human review per locked
  // failure protocol; lighter-weight than auto-invoking dealbreaker).
  // Env kill switch: PERSONA_VERIFY_HAMILTON=false to disable Hamilton.
  let verifyResult;
  let hamiltonFindings = null;
  let hamiltonError = null;
  try {
    const { prompt } = buildVerifyPrompt(bug, patch, actionPlan);
    // Parallel: Flash verify + Hamilton fan-in. Both are HTTP-bounded
    // (not subagents), so single-active rule doesn't apply here.
    const hamiltonEnabled = process.env.PERSONA_VERIFY_HAMILTON !== 'false';
    const [r, hamiltonRes] = await Promise.all([
      callPhase({ phase: 'verify', prompt }),
      hamiltonEnabled
        ? (async () => {
            try {
              const { callCouncil } = await import('../council.mjs');
              return await callCouncil({
                prompt,
                opts: {
                  personaName: 'margaret-hamilton',
                  personaPhase: 'bug-resolver-verify',
                  personaLabel: bug?.id || 'unknown',
                  timeoutMs: 180_000,
                  maxTokens: 3000,
                },
              });
    ...
    // Hamilton parsing — best-effort; never blocks Flash's verdict path
    if (hamiltonEnabled && hamiltonRes && !hamiltonRes.error) {
      try {
        const hit = (hamiltonRes.results || []).find(x => !x.error);
        if (hit) {
          const cleaned = String(hit.content || '').replace(/```json\n?|\n?```/g, '').trim();
          const parsed = JSON.parse(cleaned);
          hamiltonFindings = Array.isArray(parsed) ? parsed : [];
        }
      } catch (e) { hamiltonError = `hamilton-parse-error: ${e.message}`; }
    } else if (hamiltonRes?.error) {
      hamiltonError = `hamilton-call-error: ${hamiltonRes.error}`;
    }
    // Conflict detection: Hamilton HIGH + Flash PASS = escalate
    if (hamiltonFindings && hamiltonFindings.length > 0) {
      const hamiltonHigh = hamiltonFindings.filter(f => String(f.severity).toUpperCase() === 'HIGH');
      if (hamiltonHigh.length > 0 && verifyResult.verdict !== 'HOLD') {
        human_reasons.push(`hamilton-conflict-with-flash: Hamilton flagged ${hamiltonHigh.length} HIGH while Flash returned ${verifyResult.verdict || 'unknown'}`);
        // Attach Hamilton findings to verifyResult for the eventual PR body / audit report.
        verifyResult.hamilton_findings = hamiltonFindings;
        verifyResult.hamilton_conflict = true;
        return finalize({
          status: 'NEEDS_HUMAN',
          audit_summary: auditResult.summary,
          action_plan: actionPlan,
    ...
    if (hamiltonError) verifyResult.hamilton_error = hamiltonError;
  } catch (err) {
  ...
  // ─── Phase 7: Harden ───────────────────────────────────────────────────
  // 2026-05-25 — per AGENTS.md § "Bug class: stale-on-disk-state-from-fixed-bug":
  // when the bug-under-fix writes persistent state to disk (a summary file, a
  // cache entry, a status JSON), the harden phase MUST surface whether a
  // companion cleanup-scanner exists. A code revert alone leaves stale on-disk
  // state that downstream dashboards continue to surface as the old symptom —
  // marking the bug RESOLVED before the on-disk symptom is archived undermines
  // user trust in the resolution. The harden prompt asks: "does this bug write
  // persistent state to disk? If yes, is there a `scripts/scan-stale-<surface>.mjs`
  // sibling script that cleans the pre-fix state?" If no, the suggestion should
  // be to add one before flipping the ledger to RESOLVED.
  let hardeningResult;
  try {
