...
/**
 * Enforce the documented 0-5 item cap on data-first arrays, in place.
 * Truncate rather than reject: a 6th item is surplus, not a malformed
 * response worth burning a retry attempt on. The flags surface over-emission
 * to callers/telemetry. Shared by the data-first validator (fresh LLM output)
 * and getCachedStrategy (entries cached before the cap existed) so the two
 * paths can't drift. No-op for legacy/actions[] shapes.
 */
function _capDataFirstArrays(obj) {
  if (Array.isArray(obj.evidence) && obj.evidence.length > 5) {
    obj.evidence = obj.evidence.slice(0, 5);
    obj._evidence_truncated = true;
  }
  if (Array.isArray(obj.open_gaps) && obj.open_gaps.length > 5) {
    obj.open_gaps = obj.open_gaps.slice(0, 5);
    obj._open_gaps_truncated = true;
  }
  return obj;
}
...
export function getCachedStrategy(cacheKey, maxAgeMs = DEFAULT_MAX_AGE_MS) {
  ...
  try {
    ...
    // Entries cached before the 0-5 cap existed can carry oversized arrays —
    // cap at read time so cache hits honor the documented schema too.
    return _capDataFirstArrays(raw);
  } catch {
    return null;
  }
}
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    const result = mode === 'legacy'
      ...
      : {
          ...
          ...(parsed._evidence_truncated ? { _evidence_truncated: true } : {}),
          ...(parsed._open_gaps_truncated ? { _open_gaps_truncated: true } : {}),
          reasoning: parsed.reasoning ?? null,
          generated_at: Date.now(),
          cache_key: cacheKey,
          _grounded: grounded.metadata.mode !== 'legacy',
    ...
    if (
      ...
      try {
        // data-first mode has no actions[] — classify the evidence[]/open_gaps[]
        // prose instead (bug-2026-05-25-211: classifier silently no-oped on the
        // data-first schema because JSON.stringify(undefined) is undefined).
        const voiceInput = mode === 'legacy'
          ? parsed.actions
          : { evidence: parsed.evidence, open_gaps: parsed.open_gaps };
        const voice = await classifyVoicePurity(JSON.stringify(voiceInput), callCouncil);
