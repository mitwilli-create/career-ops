...
{"match_num": "<num>" | null, "confidence": 0.0-1.0, "reasoning": "<one short sentence>"}`;
  ...
  try {
    ...
    // Best-effort: never fail the calling triage flow. confidence is null
    // (not 0) — the model never produced a judgment to be confident about.
    return { match_num: null, confidence: null, reasoning: `sonnet-error: ${err.message}`, cost_usd: 0 };
  }
  return { match_num: null, confidence: null, reasoning: 'sonnet-parse-failure', cost_usd: 0 };
}
 ...
 *
 * `confidence` is a number ONLY when a matcher actually ran and produced a
 * judgment (exact-equal 1.0, or the Sonnet-reported value). On every
 * no-detection path (disabled / no-input / no-apps-md / no-candidates /
 * fuzzy-no-candidates / cap-exceeded-fallback / sonnet errors) it is null —
 * "not computed", never a confident 0 (silent-zero bug class).
 *
 * Method values:
...
export async function detectSurfaceAlias({ title, company, url }, opts = {}) {
  if (!ENABLED) {
    return { match: null, method: 'disabled', confidence: null };
  }
  if (!title || !company) {
    return { match: null, method: 'no-input', confidence: null };
  }

  const appsPath = opts.appsPath || APPS_PATH;
  if (!existsSync(appsPath)) {
    return { match: null, method: 'no-apps-md', confidence: null };
  }
  ...
  if (!liveAtCompany.length) {
    return { match: null, method: 'no-candidates', confidence: null };
  }
  ...
  if (!fuzzy.length) {
    return { match: null, method: 'fuzzy-no-candidates', confidence: null };
  }
