...
- **MANDATORY: \`Listed comp\` row** — exact value if JD discloses (\`$115K–$230K\`), or \`Undisclosed — estimated $X-$Y (basis: Levels.fyi / Blind / Series-stage typical)\` if not. Never omit this row. Even "Undisclosed — no estimate possible" is acceptable, but the row must exist.
...
function computeConsensus(results) {
  ...
  if (valid.length === 0) {
    // INSTANCE C (2026-05-24, GAP-RES-26 follow-up): confidence is null on the
    // failure path because `final_score: null` means no value was computed.
    // `confidence: 0` was misleading (asserts "I computed 0 confidence" when
    // reality is "I computed nothing"). Safe per caller analysis at
    // scripts/phase3b-evaluator.mjs:268 — `if (!council.consensus.final_score)`
    // early-returns before any code reads `.confidence` on the failure path.
    return { final_score: null, confidence: null, agreement: '0/0', dissent_flag: true, error: 'no valid provider results' };
  }
