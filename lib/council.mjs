...
// ─────────────────────────────────────────────────────────────────────────────
// Per-vendor monthly spend caps (cost-distribution Part 2, 2026-05-25)
//
// Read at call-time from env so tests can mutate process.env and so /deploy-
// verify / launchd can override caps without restarting Node.
//
// CAP DEFAULTS (USD/month):
...
// Set to 0 to disable cap-checking on that vendor entirely.
//
// Calculation:
...
//   3. If (month_spend + estimated_call_cost) > cap, refuse with structured
//      error: COUNCIL_CAP_REACHED: vendor=<v> month=$<n> cap=$<c> — set
...
export async function callCouncil({ prompt, models, opts = {} } = {}) {
  ...
  const tasks = fireable.map(async (name) => {
    ...
    // Clamped to [1ms, 30min]. The `rawOverride > 0` check already rejects
    // zero/negative/NaN; the Math.max(1) only guards sub-millisecond floats.
    // Hoisted out of fireOnce so the catch block below can include
    // effectiveTimeoutMs in friendly timeout-error messages.
    const rawOverride = Number(opts.timeoutMs);
    const overrideTimeoutMs = Number.isFinite(rawOverride) && rawOverride > 0
      ? Math.min(Math.max(rawOverride, 1), 1_800_000)
      : null;
    const effectiveTimeoutMs = overrideTimeoutMs ?? provider.timeout;
    ...
    // Per-vendor monthly-cap pre-flight (cost-distribution Part 2, 2026-05-25).
    // Skipped when opts.vendorCapsDisabled is true OR when the vendor cap is
    // configured to 0 (disabled). The estimated cost passed to checkVendorCap
    // is a rough envelope based on opts.maxTokens; the real spend is recorded
    // post-call via the cost-trace writer.
  ...
  return {
    ...
    missingKeys,
    totalMs: Date.now() - t0,
    report: { totalCost, totalTokens, modelCount: results.length },
  };
}
...
//                    Range honored: 1ms - 1800000 (30 min). For long-output
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === '--help' || a === '-h') {
      console.log([
        ...
        '',
        'Options:',
        '  --timeout-ms <ms>   Per-call hard ceiling (1 - 1800000 ms).',
