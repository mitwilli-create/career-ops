...
//
// Severity scope expanded 2026-05-25 (v2 interview Q11): LOW is now also
// bridged. Immediate-invocation gating moves to a separate predicate
// (isImmediateInvocationFinding) so CRIT/HIGH/MED fire bug-resolver inline
// while LOW sits in the queue until the next scheduled Mon/Thu 02:00 PT run.
const ELIGIBLE_TYPES = new Set([1, 2, 3, 4]);
const ELIGIBLE_SEVERITIES = new Set(['CRIT', 'HIGH', 'MED', 'LOW']);

// Severities that trigger immediate inline bug-resolver invocation after
// regression-guard writes findings (v2 interview Q10).
// LOW intentionally excluded — LOW findings wait for the scheduled cycle.
const IMMEDIATE_INVOCATION_SEVERITIES = new Set(['CRIT', 'HIGH', 'MED']);
...
// Decide whether a finding should trigger immediate inline bug-resolver
// invocation. CRIT/HIGH/MED → yes; LOW → no (queued for scheduled run).
// Used by regression-guard after writing the queue (v2 interview Q10).
export function isImmediateInvocationFinding(finding) {
  if (!finding) return false;
  if (!isEligibleFinding(finding)) return false;
  return IMMEDIATE_INVOCATION_SEVERITIES.has(finding.severity);
}

// Capture ±contextLines around a cited line for fingerprint-based re-resolution
// at audit time. 2026-05-25 /feedback chain — closes the drift gap so
// bug-resolver can relocate a cited line when the file shifts between intake
// and audit.
function captureLineContext(absPath, lineNum, contextLines = 5) {
  try {
    if (!existsSync(absPath) || !Number.isFinite(lineNum) || lineNum < 1) return null;
...
// The pipeline.mjs::readSource(source_surface, source_hash, line_context) call
// will then re-read the file fresh + detect drift via hash AND line-context
// fingerprint, which is the post-2026-05-25 contract.
export function mapFindingToLedgerShape(finding, id, firstSeen, lastUpdated) {
  ...
  if (sourceSurface && !sourceSurface.startsWith('github://') &&
    ...
    // Split file:line if the source_surface carries a line suffix
    const m = sourceSurface.match(/^(.+?):(\d+)$/);
    const filePart = m ? m[1] : sourceSurface;
    const linePart = m ? Number(m[2]) : (Number.isFinite(finding.line) ? finding.line : null);
    const abs = filePart.startsWith('/')
      ? filePart
      : join(REPO_ROOT, filePart);
    if (existsSync(abs)) {
      sourceHash = sha256File(abs);
      if (linePart) lineContext = captureLineContext(abs, linePart, 5);
    }
  }
  ...
  return {
    ...
    intake_metadata: {
      ...
      summary,
      // 2026-05-25 — line_context for content-fingerprint re-resolution at
      // audit phase. null when source isn't a real on-disk file (github://
      // / transcript: / memory:) or when capture failed. Sized ±5 lines.
