 ...
 *   --all           index → coverage → naming → citations → optimize → decision-doc (read-only)
...
async function runDecisionDoc({ index, coverage, naming, citations, optimize } = {}) {
...
Flags: --out <path>  --execute  --budget <usd>  --no-dumps  --json
`);
    return;
  }

  let index, coverage, naming, citations, optimize;
  ...
  if (all || flags.has('--decision-doc')) await runDecisionDoc({ index, coverage, naming, citations, optimize });
