...
function callJdKeywordScore(packSlug) {
  ...
  const artifacts = parsed.results?.[0]?.artifacts || [];
  const cv = artifacts.find(a => a.path && (a.path.includes('cv-tailored') || a.path.includes('tailored-cv') || a.path.includes('cv.md')));
...
export async function checkPackCoherence(input) {
  ...
  // 2026-05-27 — default target lowered 0.99 → 0.85. The 0.99 target was
  // aspirational ("perfect convergence") but typical LLM author confidence
  // tops out at 0.80-0.92 for healthy output. Result: every healthy pack
  // showed REJECTED final_recommendation even with all artifacts polishing
  // correctly. 0.85 is achievable + still meaningful.
  const target = opts.targetConfidence ?? 0.85;
  // Blocker floor — confidence below this is a real failure (legit REJECTED).
  // Above this but below `target` is NEEDS_HUMAN (caution). Used to be 0.85
  // (matched the target), which made ANY below-target artifact a blocker.
  const blockerFloor = opts.blockerFloor ?? 0.50;
  const packSlug = input.packSlug;
  const dataDir = input.dataPackDir;

  const perArtifact = input.perArtifact || {};
  // 2026-05-27 — Skipped artifacts (no .md source, no generator, cost-cap-
  // reached-before-artifact) are EXCLUDED from confidence aggregation +
  // REJECTED determination. They never polished; their confidence:0 in the
  // record was just a placeholder. Filtering them here means the REJECTED
  // verdict reflects ACTUALLY-POLISHED artifact quality, not "this pack is
  // incomplete." Skipped artifacts surface separately via `skipped_artifacts`
  // so the UI can prompt the user to "Run Build Pack first" when many slots
  // are missing.
  const polishedEntries = Object.entries(perArtifact).filter(([, v]) => v?.skipped !== true);
  const skippedEntries  = Object.entries(perArtifact).filter(([, v]) => v?.skipped === true);
  const perArtifactConfidence = {};
  for (const [k, v] of polishedEntries) {
    perArtifactConfidence[k] = Number(v?.confidence) || 0;
  }
  const skipped_artifacts = skippedEntries.map(([k, v]) => ({
    artifact: k,
    skip_reason: v?.skip_reason || v?.error || 'unknown',
    actionable_suggestion: v?.actionable_suggestion || null,
  }));
  ...
  // Aggregate blocking issues — ONLY from polished (non-skipped) artifacts
  const blockingIssues = [];
  for (const [name, v] of polishedEntries) {
    ...
    if ((Number(v?.confidence) || 0) < target) {
      ...
      blockingIssues.push({
        ...
        // 2026-05-27 — blocker reserved for genuinely-broken output
        // (confidence < blockerFloor = 0.50). 0.50-target range is caution
        // (NEEDS_HUMAN). Above target is no finding at all.
        severity: Number(v.confidence) < blockerFloor ? 'blocker' : 'caution',
  ...
  // Final recommendation — operates on POLISHED artifacts only (skipped excluded).
  // If every artifact was skipped, defer to NEEDS_HUMAN (no actual polish ran).
  const hasPolishedArtifacts = polishedEntries.length > 0;
  const allConfidenceOK = hasPolishedArtifacts && Object.values(perArtifactConfidence).every(c => c >= target);
  ...
  if (!hasPolishedArtifacts) {
    // Nothing polished — can't approve, can't legitimately reject. NEEDS_HUMAN.
    finalRecommendation = 'NEEDS_HUMAN';
  } else if (hasBlocker || (claim.ok && claim.pct !== null && claim.pct < 70)) {
  ...
  const summary = {
    ...
    // 2026-05-27 — skipped artifacts surfaced separately so UI can render
    // "5 of 6 artifacts skipped — run Build Pack first" when most slots are
    // missing source .md files. Excluded from REJECTED determination above.
    skipped_artifacts,
    polished_count: polishedEntries.length,
