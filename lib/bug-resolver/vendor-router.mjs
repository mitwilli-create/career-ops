...
//
// 2026-05-25 — timeouts are now env-var configurable per-phase. Background:
// bug-2026-05-25-033 (the row-resolver regex audit) hit the 120s ceiling on
// Gemini 2.5 Pro and returned NEEDS_HUMAN with no actual signal. Per locked
// decision in /feedback chain 2026-05-25, audit defaults to 900s with env
// override; research is bumped to 600s (same long-output risk); other phases
// keep their tighter defaults but accept env override too.
//
// Override pattern:
//   BUG_RESOLVER_<PHASE>_TIMEOUT_MS=NNNNNN
//
// e.g. BUG_RESOLVER_AUDIT_TIMEOUT_MS=1200000 → 20 min audit window.
const _envInt = (k, dflt) => {
  const v = Number(process.env[k]);
  return Number.isFinite(v) && v > 0 ? v : dflt;
};
export const PHASE_DEFAULTS = {
  audit:       { maxTokens: 4000, timeoutMs: _envInt('BUG_RESOLVER_AUDIT_TIMEOUT_MS',       900_000) },  // 15 min default
  research:    { maxTokens: 4000, timeoutMs: _envInt('BUG_RESOLVER_RESEARCH_TIMEOUT_MS',    600_000) },  // 10 min default
  adjudicate:  { maxTokens: 2000, timeoutMs: _envInt('BUG_RESOLVER_ADJUDICATE_TIMEOUT_MS',  60_000) },
  action_plan: { maxTokens: 3000, timeoutMs: _envInt('BUG_RESOLVER_ACTION_PLAN_TIMEOUT_MS', 90_000) },
  implement:   { maxTokens: 8000, timeoutMs: _envInt('BUG_RESOLVER_IMPLEMENT_TIMEOUT_MS',   180_000) },
  verify:      { maxTokens: 1500, timeoutMs: _envInt('BUG_RESOLVER_VERIFY_TIMEOUT_MS',      45_000) },  // Flash — faster
  harden:      { maxTokens: 3000, timeoutMs: _envInt('BUG_RESOLVER_HARDEN_TIMEOUT_MS',      60_000) },  // Flash — faster
};
