...
const BATCH_BODY_JSON_TIMEOUT_MS = 60_000;
const BATCH_BODY_TEXT_TIMEOUT_MS = 30_000;
...
export async function submitBatch(requests) {
  ...
  if (!r.ok) {
    const txt = (await readText(r, BATCH_BODY_TEXT_TIMEOUT_MS, 'batch-submit')).slice(0, 480);
    return { ok: false, error: `Batch submit HTTP ${r.status}: ${txt}` };
  }

  const j = await readJson(r, BATCH_BODY_JSON_TIMEOUT_MS, 'batch-submit');
...
export async function pollBatch(batchId) {
  ...
  const j = await readJson(r, BATCH_BODY_JSON_TIMEOUT_MS, 'batch-poll');
  const state = readState();
...
export async function fetchBatchResults(batchId) {
  ...
  const text = await readText(r, BATCH_BODY_JSON_TIMEOUT_MS, 'batch-results');
