...
//   data/persona-review-spend.jsonl    — catalog-aware-personas (added
//                                          2026-05-28 with persona-system
//                                          Wave 1; ~$5-15/month nominal)
//
...
const SPEND_FILES = [
  ...
  join(DATA_DIR, 'persona-review-spend.jsonl'),
];
...
/**
 * Append a spend record to data/persona-review-spend.jsonl.
 *
 * Used by Wave 2 wire-ins: /deploy-verify Phase 3.5, /bug-resolver verify
 * (Hamilton fan-in), /omega-steward Phase 4 (DHH/Howard/Hickey on proposals),
 * /council --persona, /personas batch. Costs sum into V2_BUDGET_USD=$300.
 *
 * Cross-fork-leak note: this ledger is gitignored (see .gitignore) and the
 * cross-fork-leak guard treats it as a sensitive path. Findings reports
 * elsewhere may reference spend totals only — never inline persona prompts
 * or finding content.
 *
 * @param {{ usd: number, persona: string, phase: string, label?: string }} entry
 */
export function recordPersonaReviewSpend({ usd, persona, phase, label = '' }) {
  if (typeof usd !== 'number') throw new Error('recordPersonaReviewSpend: usd must be a number');
  if (!persona) throw new Error('recordPersonaReviewSpend: persona name required');
  if (!phase) throw new Error('recordPersonaReviewSpend: phase required');
  const personaSpendPath = join(DATA_DIR, 'persona-review-spend.jsonl');
  if (!existsSync(DATA_DIR)) mkdirSync(DATA_DIR, { recursive: true });
  const rec = {
    date: ptDateStamp(),
    timestamp: new Date().toISOString(),
    ...
    phase,
    label,
    source: 'council-personas',
  };
  try {
    appendFileSync(personaSpendPath, JSON.stringify(rec) + '\n');
  } catch { /* never crash on log write */ }
}
