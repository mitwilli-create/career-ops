...
// PR-04 (2026-05-25) — Synthesis mode feature flag.
//   - 'corpus-grounded' (default) → routes through lib/team-health-synthesis.mjs
//     which uses existing hm-intel + Mitchell's personality + profile corpus
//     via lib/ground-prompt.mjs + Sonnet 4.6. No new scrapers required.
//   - 'scraper-based' (future)    → routes through the legacy rawSignals path
//     below (still works when opts.rawSignals is passed).
//   - Set TEAM_HEALTH_SYNTHESIS_MODE=scraper-based to bypass new path.
export const TEAM_HEALTH_SYNTHESIS_MODE = (process.env.TEAM_HEALTH_SYNTHESIS_MODE || 'corpus-grounded').toLowerCase();
 ...
 * REJECTED by PR-04 strategy adjudication 2026-05-25 in favor of corpus-grounded
 * path — see lib/team-health-synthesis.mjs).
...
export async function synthesizeTeamHealth(companyName, opts = {}) {
  ...
  // PR-04 (2026-05-25) — Corpus-grounded synthesis path. Default ON. Routes
  // to lib/team-health-synthesis.mjs which uses existing hm-intel signals +
  // personality + profile corpus via lib/ground-prompt.mjs. No scrapers
  // required. Set TEAM_HEALTH_SYNTHESIS_MODE=scraper-based to bypass.
  if (TEAM_HEALTH_SYNTHESIS_MODE === 'corpus-grounded' && !opts.rawSignals) {
    try {
      const synthMod = await import('./team-health-synthesis.mjs');
      const synthResult = await synthMod.synthesizeTeamHealth({
        slug,
        opts: {
          companyName,
          roleName: opts.roleName || '',
          rowId: (opts.rowNums && opts.rowNums[0]) || null,
          force: opts.force,
          budgetUsd: opts.budgetUsd,
          callCouncil: opts.callCouncil,
        },
      });
      return {
        status: synthResult.status === 'SYNTHESIZED' || synthResult.status === 'CACHED' ? 'SYNTHESIZED' : synthResult.status,
        record: synthResult.record,
        cost_usd_estimate: synthResult.cost_usd || 0,
        next_action: synthResult.reason || 'corpus-grounded synthesis complete',
      ...
      return {
        ...
        record: null,
        cost_usd_estimate: 0,
        next_action: 'corpus-grounded synthesis failed: ' + (err && err.message ? err.message : String(err)),
      };
    }
  }

  // No rawSignals AND scraper-based mode — return structured stub for
  // backward compatibility. (PR-04 default routes via corpus-grounded above.)
  if (!opts.rawSignals || typeof opts.rawSignals !== 'object') {
    return {
      ...
      cost_usd_estimate: 0,
      next_action: [
        'TEAM_HEALTH_SYNTHESIS_MODE=scraper-based requires opts.rawSignals.',
        'To synthesize via legacy path: pass opts.rawSignals = { glassdoor: {...}, indeed: {...}, blind: {...}, "levels.fyi": {...} } per the schema in lib/team-health.mjs header.',
        'For corpus-grounded synthesis (default), unset TEAM_HEALTH_SYNTHESIS_MODE or set to "corpus-grounded" — uses existing hm-intel + Mitchell\'s personality + profile corpus.',
      ].join(' '),
    };
  }
