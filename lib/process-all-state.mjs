...
//     populate ALL of them. Presence-required fields (type/status/phase)
//     default to typed sentinels so the SSE never sees undefined.
//   - Not-yet-computed COUNT metrics (triage_advanced / processed /
//     pending_before / rounds_completed) default to `null`, NOT 0. `null`
//     means "the phase that computes this hasn't run"; a genuine 0 means
//     "the phase ran and the count is zero". A 0-fill collapses those two
//     into a lie the dashboard renders as "0 succeeded" when it means
//     "not yet run" (bug class: sentinel-treated-as-truthy, numeric analogue).
//     Readers MUST gate on isMetricComputed()/`!= null`, not `|| 0`.
//     (2026-07-06 — Qodo PR #385 finding 2 migration.)
//
// Cross-references:
...
// forgets to provide one, writeProcessState() inserts a default: a typed
// sentinel for presence-required fields, or `null` for not-yet-computed
// count metrics (see COUNT_METRIC_FIELDS + isMetricComputed below).
...
// Count metrics that default to `null` ("not yet computed") rather than 0.
// A 0-fill here is indistinguishable from a genuine zero — the exact lie the
// dashboard surfaced as "0 succeeded" for a job that had not run its counting
// phase. Consumers gate on isMetricComputed() before treating the value as real.
export const COUNT_METRIC_FIELDS = [
  'triage_advanced',
  'processed',
  'pending_before',
  'rounds_completed',
...
/**
 * Is a process-state count metric genuinely computed?
 *
 * `null` (this migration) or `undefined` (records written before the field
 * existed) both mean "not yet computed" — NOT "genuinely zero". A reader that
 * needs to distinguish "0 succeeded" from "hasn't run" MUST gate on this
 * instead of a `|| 0` / truthiness coalesce.
 *
 * A count metric is only "computed" if it is a finite number. NaN / Infinity /
 * strings are malformed, not computed — treating them as computed would leak
 * "NaN" (or arbitrary text) into the state file and UI. They render as
 * not-computed ("—") instead, preserving the count-metric contract.
 *
 * @param {*} v
 * @returns {boolean}
 */
export function isMetricComputed(v) {
  return typeof v === 'number' && Number.isFinite(v);
}

/**
 * Render a process-state count metric for display. Not-computed (null/undefined)
 * renders as the `notRun` sentinel (default "—", matching the dashboard's
 * established empty-cell convention) instead of a fake "0". A genuine 0 renders
 * as "0". This is the single source of truth for the null → "—" mapping so the
 * server SSE consumer, the client renderer, and the data-truth-audit test all
 * agree.
 *
 * @param {*} v
 * @param {{ notRun?: string }} [opts]
 * @returns {string}
 */
export function formatProcessMetric(v, { notRun = '—' } = {}) {
  return isMetricComputed(v) ? String(v) : notRun;
}
 ...
 * @param {object} opts.metrics — { triage_advanced?, processed?, pending_before?, pending_after?, rounds_completed?, last_batch_id?, last_batch_result? }. Omitted count metrics default to `null` (not-yet-computed), never 0. Pass an explicit 0 to record a genuine zero.
...
export function writeProcessState({
  ...
  // Build the record. Presence-required fields (type/status/phase) ALWAYS
  // present with typed sentinels (never undefined). Not-yet-computed COUNT
  // metrics default to `null` — never 0 — so "not run" stays distinguishable
  // from a genuine zero (see COUNT_METRIC_FIELDS + isMetricComputed).
  // Extra fields layer on top.
  const record = {
    ...prior,
    job_id: jobId,
    ...
    // Count metrics: metric wins if provided (incl. a genuine 0), else prior if
    // computed, else `null` = not-yet-computed. NEVER 0-fill (2026-07-06).
    triage_advanced: metrics.triage_advanced != null ? metrics.triage_advanced : (prior.triage_advanced != null ? prior.triage_advanced : null),
    processed: metrics.processed != null ? metrics.processed : (prior.processed != null ? prior.processed : null),
    pending_before: metrics.pending_before != null ? metrics.pending_before : (prior.pending_before != null ? prior.pending_before : null),
    ...
    rounds_completed: metrics.rounds_completed != null ? metrics.rounds_completed : (prior.rounds_completed != null ? prior.rounds_completed : null),
