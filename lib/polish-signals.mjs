...
export async function harvestPolishSignals({ slug, company, role, jdText, opts = {} } = {}) {
  ...
  const councilPrompt = [
    ...
    trimmedCv,
    ``,
    `### Mitchell's voice brief (canonical metrics & banned-phrase checklist — DO NOT invent new metrics)`,
    trimmedVoice,
    ``,
    `## Your task`,
    ...
    `- Honor Mitchell's banned-phrase checklist: no "delve", "tapestry", "leverage" (verb), "passionate", exclamation marks.`,
  ].join('\n');
