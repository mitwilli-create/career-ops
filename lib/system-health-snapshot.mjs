...
export function checkLaunchdPlists(root) {
  ...
  try { launchctlListOut = execSync('launchctl list 2>/dev/null', { encoding: 'utf-8', timeout: 10_000 }); }
 ...
 *  Uses `-sTCP:LISTEN` to filter to LISTEN-state sockets only. The prior
 *  pipeline (`| head -3`) returned ESTABLISHED connections from the
 *  cloudflared tunnel + dropped the LISTEN line, producing false-negative
 *  "not listening" results (B4.1 fix 2026-05-20). */
export function checkDashboardServer(port = 3097) {
  try {
    const out = execSync(`lsof -nP -iTCP:${port} -sTCP:LISTEN 2>&1`, { encoding: 'utf-8', timeout: 10_000 });
