...
// ── Response schema (consumed by buildGroundedPrompt) ────────────────────────

const STRATEGY_CEILING_RESPONSE_SCHEMA = [
  `{`,
...
export async function computeStrategyCeiling({
  ...
  // The grounded.cacheKey embeds rowId + metricKey + role + company + corpus
  // mtimes — strictly stronger than the legacy buildCacheKey, but we keep the
  // legacy export for any external caller still using it.
  const grounded = buildGroundedPrompt({
    task: metricKey,
    rowId, role, company, jdText, hmIntel,
    metricKey, currentValue,
    responseSchema: STRATEGY_CEILING_RESPONSE_SCHEMA,
  });
  const cacheKey = grounded.cacheKey;
  ...
  // γ GAMMA fix 2026-05-19 (audit CRIT-3) + PR-08 softening 2026-05-25:
  ...
  // New behavior: surface a "Retry now" CTA (hits PR-03's /api/drawer/auto-enrich)
  // backed by a per-row+day cap (default 3 attempts/day) so the drawer doesn't
  // burn unbounded $1 retries on a chronic LLM outage.
  ...
  const retryInfo = _retryAvailability(metricKey, rowId);
  const degraded = {
    current: currentValue,
