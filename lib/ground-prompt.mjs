...
function _loadProfileCorpus() {
  const filesToTry = ['cv.md', 'article-digest.md', 'modes/_profile.md', 'config/profile.yml'];
  const parts = [];
  const filesUsed = [];
...
function _buildSystemPrompt({ task }) {
  return [
    ...
    `  3. Banned vocabulary (replace with plain English): "leverage", "synergy", "deep dive", "ideate", "circle back", "touch base", "bandwidth", "moving forward", "reach out". These read as corporate noise and break voice match.`,
