...
// ── Playwright DOM acquisition (Qodo B8 refactor, 2026-07-06) ────────────────
// The generic fallback previously decided liveness from a bare fetch + HTML
// phrase scan, which misreads SPAs (Ashby/Lever/Workday render client-side —
// the fetched HTML is an empty shell). Per the repo's Offer-Verification rule
// (rule 1670182), the DOM is acquired via Playwright and the DECISION is
// delegated to the pure classifyLiveness() in liveness-core.mjs — the same
// logic check-liveness.mjs uses. The {result, reason} contract is unchanged.
//
// Serialization: a module-level promise chain guarantees the shared page is
// never used by two probes at once ("never Playwright in parallel").
// Lifecycle: the browser launches lazily and auto-closes after 60s idle
// (unref'd timer) so long-lived importers (dashboard-server) don't hold a
// chromium process open between sweeps. closeLivenessBrowser() is exported
// for deterministic teardown in tests/sweeps.
let _browser = null;
let _queue = Promise.resolve();
let _idleClose = null;

const PLAYWRIGHT_DISABLED = process.env.LIVENESS_DISABLE_PLAYWRIGHT === 'true';
...
export async function closeLivenessBrowser() {
  if (_idleClose) { clearTimeout(_idleClose); _idleClose = null; }
  const b = _browser;
  _browser = null;
...
function scheduleIdleClose() {
  if (_idleClose) clearTimeout(_idleClose);
  _idleClose = setTimeout(() => { closeLivenessBrowser(); }, 60_000);
  if (typeof _idleClose.unref === 'function') _idleClose.unref();
}

// Collect { status, finalUrl, bodyText, applyControls } from a real DOM —
// same acquisition pattern as check-liveness.mjs::checkUrl.
async function acquireDomSnapshot(url) {
  ...
  try {
    ...
    return { status, finalUrl, bodyText, applyControls };
  } finally {
...
async function probeViaPlaywright(url) {
  // Chain onto the queue so probes never run in parallel.
  const run = _queue.then(async () => {
    const snapshot = await acquireDomSnapshot(url);
    return classifyLiveness(snapshot);
  });
  // Keep the chain alive even when this probe fails.
  _queue = run.catch(() => {});
  return run;
}
...
export async function verifyApplyNowLink(url) {
  ...
  // Generic path — acquire a real DOM via Playwright and delegate the
  // decision to the pure classifyLiveness() (Qodo B8 refactor). A navigation
  // error stays 'uncertain' (NOT 'expired') because callers auto-discard
  // tracker rows on 'expired' — a transient network blip must never do that.
  if (!PLAYWRIGHT_DISABLED) {
    try {
      const verdict = await probeViaPlaywright(url);
      return { result: verdict.result, reason: `playwright: ${verdict.reason}` };
    } catch (err) {
      const msg = String(err?.message || err).split('\n')[0];
      // Playwright missing/unlaunchable (headless CI, batch pipe mode) →
      // fall through to the legacy fetch heuristic rather than failing the
      // whole sweep. Navigation-level errors return 'uncertain' below.
      if (!/Cannot find (module|package)|browserType\.launch|Executable doesn't exist/i.test(msg)) {
        return { result: 'uncertain', reason: `playwright failed: ${msg.slice(0, 80)}` };
      }
    }
  }

  // Legacy fetch fallback — only reached when Playwright is unavailable
  // (LIVENESS_DISABLE_PLAYWRIGHT=true, missing browser binary, batch pipe
  // mode). Less reliable on SPAs; kept so heartbeat/liveness-sweep degrade
...
export function markRowAsExpired(rowNum, urlReason, opts = {}) {
  ...
  // atomic tmp+rename — the tracker is the canonical pipeline record; a crash
  // mid-write must never truncate it (rule 1671590).
