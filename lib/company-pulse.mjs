...
export function parsePulseReport(reportPath, slug) {
  ...
  // Bridge output uses numbered headings: "## 1. Hiring signals (what they're investing in)"
  // These patterns match both numbered ("1. Hiring signals") and bare ("Hiring Signals") forms.
  const hiringSection  = extractSection('(?:\\d+\\.\\s+)?[Hh]iring[\\s./]*[Ss]ignals?');
  const leaderSection  = extractSection('(?:\\d+\\.\\s+)?[Ll]eader[\\s./\\-]*/\\s*[Mm]edia(?:\\s+[Ss]ignals?)?|(?:\\d+\\.\\s+)?[Ll]eader\\s+[Mm]edia');
  const teamSection    = extractSection('(?:\\d+\\.\\s+)?[Tt]eam\\s+[Ee]vidence');
  const citationSection = extractSection('(?:\\d+\\.\\s+)?(?:[Cc]itations?|[Ss]ources?)');
...
<div class="pulse-card" role="region" aria-label="Company Pulse — ${esc(pulse.company_slug)}">
  ...
  <section class="pulse-section">
    ...
    ${renderItems(pulse.leader_media)}
  </section>
