...
**Every company MUST have `careers_url` in portals.yml.** If it's missing, look it up once, save it, and use it in future scans.
...
5. **Level 2 — ATS APIs / feeds** (parallel):
   ...
   c. For **Ashby**, send POST with:
      ...
      - GraphQL query from `jobBoardWithTeams` + `jobPostings { id title locationName employmentType compensationTierSummary }`
...
6b. **Filter by location (optional)** using `location_filter` from `portals.yml`:
   ...
   - Empty location on a posting → passes (don't penalize missing data)
...
7.5. **Verify liveness of WebSearch results (Level 3)** — BEFORE adding to pipeline:
   ...
   WebSearch results can be stale (Google caches results for weeks or months). To avoid evaluating expired postings, verify with Playwright each new URL from Level 3. Levels 1 and 2 are inherently real-time and don't require this check.

   For each new Level 3 URL (sequential — NEVER Playwright in parallel):
   a. `browser_navigate` to the URL
   b. `browser_snapshot` to read the content
   c. Classify:
      - **Active**: job title visible + role description + Apply/Submit control visible in the main content. Don't count generic header/navbar/footer text.
...
Every company in `tracked_companies` must have `careers_url` — the direct URL to its job listings page. This avoids looking it up on every scan.

**RULE: Always use the company's own corporate URL; fall back to the ATS endpoint only if no corporate careers page exists.**

The `careers_url` should point to the company's own jobs page whenever available. Many companies use Workday, Greenhouse, or Lever under the hood but expose job IDs only through their corporate domain. Using the ATS direct URL when a corporate page exists can cause false 410 errors because job IDs don't match.
...
Fallback: if you only have the direct ATS URL, navigate to the company website first and locate its corporate careers page. Only use the direct ATS URL if the company has no corporate careers page of its own.
