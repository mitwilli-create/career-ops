...
export const CACHES = [
  ...
  // ── Layer 2: Sonnet refresh (per-row, tier-stratified) ──
  {
    ...
    dedupScope: 'row',
    // 2026-07-09 recalibration: 0.8 rejected every observed hm_intel_delta
    // output. Live run showed Sonnet emits 4-8 URLs for ~1800-2600-token
    // hiring-manager-delta records (~0.2-0.3 URLs/100tk); the 0.8 floor
    // demanded 15-21. Lowered to 0.15 (keeps the anti-hallucination spirit —
    // still must cite sources — without demanding research-grade density on a
    // delta record). Mirrors the 2026-05-19 positioning recalibration.
    minCitationsPer100Tokens: 0.15,
    // 2026-07-09: rich Anthropic/Databricks rows truncated writer output at the
    // orchestrator's 3500 default (stop_reason=max_tokens → WRITER_FAILED). The
    // #413 fix raised role_enrichment/positioning/toxicity but missed this slot.
    maxTokens: 8000,
  },
  {
    ...
    // 2026-07-08: research-heavy composite record; the orchestrator's 3500
    // default truncated writer output mid-JSON (finish_reason=length →
    // WRITER_FAILED with the pre-fix empty error string).
    maxTokens: 8000,
  },
  {
    id: 'positioning',
    ...
    // 2026-07-09: even 0.15 blocked live outputs (1 URL / ~3489 tokens =
    // 0.029/100tk). Positioning is corpus-grounded strategic synthesis, not
    // external-research; lowered to 0.025 so a genuinely low-citation craft
    // output completes while still requiring ≥1 source for typical sizes.
    minCitationsPer100Tokens: 0.025,
    // 2026-07-08: positioning row 50 hit the 3500-token orchestrator default
    // and failed unparseable (WRITER_FAILED, empty pre-fix error). 6000 gives
    // Mitchell-voice synthesis room without inviting rambling output.
    maxTokens: 6000,
  },
  {
    id: 'role_enrichment',
    ...
    // 2026-07-08: full role_enrichment records (relocation + benefits +
    // sentiment + people + per-field source URLs) exceed the orchestrator's
    // 3500-token default — every 2026-07-08T17 write was either truncated
    // mid-JSON (WRITER_FAILED) or schema-partial (VERIFIER_REJECTED).
    maxTokens: 8000,
  },
  ...
  // ── Layer 3: Deep Research + council (event-triggered + scheduled rotation) ──
  {
    ...
    filePattern: '{slug}.json',
    keyFromRow: (r) => slugify(r.company + '-' + r.role),
    refreshHandler: 'node scripts/agents/intel-refresh.mjs --slots hm-intel --rows {slug} --mode deep-council-7',
    costEstimate: 30,
  ...
  },
  {
    ...
    filePattern: '{company_slug}.json',
    keyFromRow: (r) => slugify(r.company),
    refreshHandler: 'node scripts/agents/intel-refresh.mjs --slots company-pulse --company-slug {company_slug}',
    costEstimate: 2,
    hardMaxTtlDays: 7,
    provider: 'anthropic-sonnet',
    dedupScope: 'company',
