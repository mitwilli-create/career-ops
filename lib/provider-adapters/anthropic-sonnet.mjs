...
export async function refresh(cache, row, opts = {}) {
  ...
  const varyingPrompt = opts.promptBuilder
    ...
    : defaultPrompt(cache, row, opts.schemaHint);
  ...
  // 2026-07-08: truncated or unparseable output must carry explicit errors[].
  // Previously ok:!!parsed returned no errors, so the orchestrator logged
  // "WRITER FAILED: " with an empty string and persisted no diagnostic
  // (bug class: findings-exit-code-conflated-with-spawn-failure).
  if (stopReason === 'max_tokens') {
    return {
      ok: false,
      errors: [
        `Anthropic output TRUNCATED at max_tokens=${opts.maxTokens || 3000} (stop_reason=max_tokens, ` +
        `raw_content_length=${response.content.length}). ` +
  ...
  if (!parsed) {
    const tail = response.content.slice(-200).replace(/\s+/g, ' ');
    return {
      ok: false,
      errors: [
        `Anthropic output not parseable as JSON (stop_reason=${stopReason || 'unknown'}, ` +
        `raw_content_length=${response.content.length}). Tail: "...${tail}"`,
      ],
      costUsd: response.costUsd || 0,
      providerMetadata,
      sourceUrls,
...
function defaultPrompt(cache, row, schemaHint = null) {
  // 2026-07-08: pin the writer to the prior cache's structure when the
  // orchestrator provides a skeleton — schema-blind prompts caused every run
  // to invent a new structure, which the cross-architecture verifier then
  // (correctly) rejected as schema drift.
