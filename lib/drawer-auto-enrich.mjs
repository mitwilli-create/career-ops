...
import { existsSync, mkdirSync, readFileSync, writeFileSync, readdirSync, statSync, unlinkSync, appendFileSync } from 'node:fs';
...
export const SLOT_COST_ESTIMATES = Object.freeze({
  ...
  // PR-07 (apply-now UX audit 2026-05-25): story-card slot added but
  // deferred — the story-card generator depends on a story-bank.md
  // restructure that ships in a future PR. Cost estimate set to ~$0.40
  // (Sonnet 4.6 — story expansion is a single-shot generation) so the
  // CTA can preview the cost when the generator lands.
  'story-card': 0.40,
});
...
export const SLOT_ETA_SECONDS = Object.freeze({
  ...
  // PR-07 (2026-05-25): story-card placeholder — deferred until story-bank.md
  // restructure ships. ETA reserved for future generator (Sonnet 4.6 single-shot).
  'story-card': 30,
});
...
export function slotCachePath(rowId, slot, slug) {
  ...
  // PR-07 (2026-05-25): story-card cache reserves a slot under
  // dashboard/stories/<slug>.html (matches the existing render-time
  // check at scripts/build-dashboard.mjs:_storyChildPath).
  if (slot === 'story-card') return join(REPO_ROOT, 'dashboard', 'stories', `${slug}.html`);
  return null;
}
...
function slotDispatchCommand(rowId, slot) {
  ...
  if (slot === 'story-card') {
    // PR-07 (2026-05-25): deferred. The story-card generator depends on a
    // story-bank.md restructure that ships in a future PR — see the deferred
    // reason in dispatchAutoEnrich() below.
    return null;
  }
...
export async function dispatchAutoEnrich({ rowId, slot, confirmed_at, slug = null, force = false } = {}) {
  ...
  if (!dispatchCmd) {
    // Deferred slots — generators not yet shipped. The CTA still surfaces
    // (PR-07 story-card stub etc.) so the operator sees the gap + can
    // plan the follow-up sprint.
    // PR-07 (2026-05-25): also return `deferred: true` + `deferred_reason`
    // as top-level keys (in addition to the existing state/reason shape)
    // so client-side renderers can opt for the shorter contract without
    // reading state === 'deferred' first.
    let reason;
    if (slot === 'team-health') {
      reason = 'Team-health synthesis queued behind PR-04 ship';
    } else if (slot === 'story-card') {
      reason = 'story-bank.md restructure PR pending';
    } else {
      reason = `slot "${slot}" not yet implemented`;
    }
    return {
      ok: true,
      state: 'deferred',
  ...
  try {
    ...
    // Best-effort: capture stdout/stderr to log file. (Top-level ESM import —
    // `require` is undefined in .mjs and the old try/catch silently swallowed
    // it, so the log file was never written and spawn failures were invisible.)
    ...
    // Watchdog: the child is detached, so nothing would ever reap a hung
    // enrichment run — the slot lock would be held forever and the drawer
    // would show "enriching" indefinitely. Kill the whole process group
    // (detached → own group, negative pid) after the ceiling. unref'd so the
    // timer never keeps the server's event loop alive.
    const enrichTimeoutMs = Math.min(
      Math.max(parseInt(process.env.DRAWER_ENRICH_TIMEOUT_MS || '1800000', 10) || 1_800_000, 60_000),
      7_200_000
    ); // default 30 min, clamp [1min, 2h]
    const enrichWatchdog = setTimeout(() => {
      try { appendFileSync(logPath, `[watchdog] enrichment exceeded ${enrichTimeoutMs}ms — killing process group\n`); } catch {}
      try { process.kill(-proc.pid, 'SIGTERM'); } catch { try { proc.kill('SIGTERM'); } catch {} }
      setTimeout(() => {
        try { process.kill(-proc.pid, 'SIGKILL'); } catch { try { proc.kill('SIGKILL'); } catch {} }
      }, 10_000).unref();
    }, enrichTimeoutMs);
    enrichWatchdog.unref();
