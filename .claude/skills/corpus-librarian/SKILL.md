...
user_invocable: true
args: mode
argument-hint: "[index | coverage | naming | citations | optimize | decision-doc | all | apply] (default: all)"
---
...
| `all` (default) | `index → coverage → naming → citations → optimize → decision-doc`. **Read-only — no mutations.** | all of the above | $0 |
...
Flags: `--out <path>` (override decision-doc path) · `--execute` (with `--apply`, actually mutate) · `--budget <usd>` (optimize cap) · `--no-dumps` (skip the giant artifact dirs) · `--json`.
