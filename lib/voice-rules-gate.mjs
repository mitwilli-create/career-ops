 ...
 *   2. No em dashes          — em-dash count must be 0 (the — character is an
 *                              AI tell, banned in outward materials). Replaces
 *                              the retired 40-50% density mandate (2026-06-13).
...
export const VOICE_RULES = {
  ...
  2:  { name: 'no_em_dashes',               critical: false, weight: 1.0 },
...
export function checkEmDashDensity(text) {
  // 2026-06-13: INVERTED. Em dashes (the — / U+2014 character) are banned in all
  // outward, submitted, spoken, or rendered materials. They read as an AI tell.
  // Target is ZERO; any em dash fails this rule. Replaces the retired 40-50%
  // density mandate. See feedback_no_em_dashes_in_materials. Function name kept
  // for back-compat with importers; rule name is now no_em_dashes.
  const sentences = sentencesOf(text);
  const total = (String(text || '').match(/—/g) || []).length;
  if (sentences.length === 0) {
    return { rule: 2, name: VOICE_RULES[2].name, critical: false, pass: total === 0, score: total === 0 ? 1 : 0, density_pct: 0, em_dash_count: total, evidence: total === 0 ? 'no em dashes (empty input)' : `${total} em-dash(es) found — target is 0` };
  }
  ...
  return {
    ...
    pass,
    score,
    density_pct: Math.round((withDash / sentences.length) * 100),
    em_dash_count: total,
    sentence_count: sentences.length,
    sentences_with_dash: withDash,
    evidence: pass ? 'no em dashes (target: 0 for outward materials)' : `${total} em-dash(es) across ${withDash}/${sentences.length} sentences — target is 0 for outward materials`,
  };
}
