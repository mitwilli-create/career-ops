...
function writeJsonSafe(path, data) {
  // tmp+rename via the shared helper — a crash mid-write can no longer leave
  // a truncated state file (Qodo B5 atomic-write sweep).
  try {
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  ...
  if (isDiffOnce) {
    ...
    // Keep the returned stop() and call it on signals so the fs watcher /
    // poll interval is torn down deterministically (rule 1671683) instead of
    // relying on process.exit to reap live timers.
    const stop = watchQueueChanges(defaultCallback, { debounceMs: 60_000 });
    const shutdown = (sig) => {
      console.log(`\n[queue-watcher] stopping (${sig})`);
      try { stop(); } catch {}
      process.exit(0);
    };
