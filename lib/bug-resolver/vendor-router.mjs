...
// Phase → vendor binding. Quality-first per the locked Q>C>S decisions
// in data/bug-resolver-plan.md.
export const PHASE_VENDORS = {
  audit:       'google:gemini-2.5-pro',   // long context + grounded
  research:    'google:gemini-2.5-pro',   // same vendor as audit
  adjudicate:  'xai:grok-4',              // independent reasoning bias
  action_plan: 'xai:grok-4',              // same vendor as adjudicate
  implement:   'openai:gpt-5',            // strongest non-Anthropic code
  verify:      'google:gemini-2.5-pro',   // cheap pattern-match (uses base tier)
  harden:      'google:gemini-2.5-pro',   // markdown generation
};

// Per-phase default opts. maxTokens accounts for reasoning-model overhead
// (Gemini 3.1 Pro / GPT-5 / Grok-4 all burn thinking tokens before output —
...
export function isVendorDisabled(vendor) {
  // env-flag check
  const envFlag = vendorEnvDisableFlag(vendor);
  if (envFlag && /^(true|1|yes|on)$/i.test(process.env[envFlag] || '')) return true;
  // circuit-breaker
