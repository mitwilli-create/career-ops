 ...
 * - Total-comp targets are example defaults; configure your own floor and range
...
const WEALTH_BODY_JSON_TIMEOUT_MS = 90_000;
const WEALTH_BODY_TEXT_TIMEOUT_MS = 30_000;
...
const FLOOR_ABSOLUTE = 150_000;   // example default: walk regardless of equity
const FLOOR_PREFERRED = 175_000;  // example default: equity-trade floor
const TC_TARGET_LOW  = 200_000;   // example default
const TC_TARGET_HIGH = 275_000;   // example default
...
export async function applyWealthLens(comp = {}, role = {}, opts = {}) {
  ...
  const prompt = [
    ...
    `Today is ${todayIso} PT. The candidate has a $${Math.round(FLOOR_ABSOLUTE/1000)}K absolute walk-line, $${Math.round(TC_TARGET_LOW/1000)}K–$${Math.round(TC_TARGET_HIGH/1000)}K TC target, and prefers pre-IPO equity at frontier-AI labs (Series C+).`,
    ``,
  ...
  try {
    ...
    const client = llmClient ?? {
      call: async (p) => {
        ...
        if (!r.ok) throw new Error(`HTTP ${r.status}: ${(await readText(r, WEALTH_BODY_TEXT_TIMEOUT_MS, 'wealth-lens')).slice(0, 200)}`);
        const j = await readJson(r, WEALTH_BODY_JSON_TIMEOUT_MS, 'wealth-lens');
  ...
  if (peerContext && typeof peerContext.getPeerContext === 'function') {
    ...
    benchmarks = peerValues.length >= 2
      ...
      : {
          ...
          p90: current > 0 ? Math.max(current * 1.35, 300_000) : 325_000,
        };
  } else {
    benchmarks = {
      p50: current > 0 ? Math.max(current, 240_000) : 265_000,
