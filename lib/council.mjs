...
// ─────────────────────────────────────────────────────────────────────────────
// Pricing-map ↔ dispatch-table referential integrity (2026-06-05)
//
// Most MODEL_COST_RATES keys above are ALSO PROVIDERS dispatch slots. Two are
// intentionally PRICE-ONLY: they record the blended rate of a model reached only
// via auto-escalation through another slot, and are NOT directly dispatchable:
//   - openai:gpt-5-5        → reached via the 'openai:gpt-5' slot (escalates gpt-5 → gpt-5.5)
//   - google:gemini-3-1-pro → reached via 'google:gemini-2.5-pro' (escalates to gemini-3.1-pro-preview)
//
// Before 2026-06-05, callCouncil() silently dropped ANY requested id with no
// PROVIDERS block (bare `return false`, no missingKeys entry). It now FAILS LOUD
// (stderr WARN + a missingKeys{ undispatchable:true } entry). The allowlist below
// lets the two legitimate escalation-target rates coexist with that guard, and
// tests/council-dispatch-integrity.test.mjs pins both invariants so a future
// price-without-dispatcher addition trips a test instead of silently dropping.
// Bug class: pricing-map-entry-without-dispatch-block (silent model drop).
// ─────────────────────────────────────────────────────────────────────────────
export const ESCALATION_TARGET_PRICE_ONLY = new Set([
  'openai:gpt-5-5',
  'google:gemini-3-1-pro',
]);

// When a caller requests a price-only escalation target, point them at the
// dispatchable slot that actually reaches it. Used by callCouncil's fail-loud warn.
const ESCALATION_SLOT_HINT = {
  'openai:gpt-5-5': 'openai:gpt-5',
  'google:gemini-3-1-pro': 'google:gemini-2.5-pro',
};

// Every dispatchable provider:model id (= every PROVIDERS key). Exported so tests
// and callers can assert/check dispatchability without importing the PROVIDERS
// object itself (which holds live network call fns).
export const DISPATCHABLE_MODEL_IDS = new Set(Object.keys(PROVIDERS));

/** True iff `id` has a PROVIDERS dispatch block (i.e. callCouncil can fire it). */
export function isDispatchable(id) {
  return DISPATCHABLE_MODEL_IDS.has(id);
}
...
export async function callCouncil({ prompt, models, opts = {} } = {}) {
  ...
  const fireable = wantedModels.filter(name => {
    ...
    if (!provider) {
      // FAIL LOUD (2026-06-05): a requested model id with no PROVIDERS dispatch
      // block was previously dropped here with a bare `return false` — no error,
      // no missingKeys entry. A caller passing a priced-but-undispatchable id
      // (e.g. 'openai:gpt-5-5', a MODEL_COST_RATES escalation-target rate reached
      // only via the 'openai:gpt-5' slot) saw a silently short/empty lineup.
      // Surface it: stderr WARN + a structured missingKeys{ undispatchable } entry
      // so printCouncil() and programmatic consumers both see the drop.
      // Bug class: pricing-map-entry-without-dispatch-block (silent model drop).
      const priced = Object.prototype.hasOwnProperty.call(MODEL_COST_RATES, name);
      const suggestion = ESCALATION_SLOT_HINT[name] || null;
      const reason = priced
        ? (suggestion
            ? `priced as an escalation-target rate but not directly dispatchable — use "${suggestion}" (that slot auto-escalates at runtime)`
            : 'priced in MODEL_COST_RATES but has no PROVIDERS dispatch block')
        : 'unknown model id — not in the PROVIDERS dispatch table';
      try {
        process.stderr.write(`[council] WARN: requested model "${name}" is not dispatchable — ${reason}; dropping from lineup\n`);
      } catch { /* stderr best-effort */ }
      missingKeys.push({ model: name, missingEnvVar: 'N/A (not dispatchable)', undispatchable: true, reason, suggestion });
      return false;
    }
...
export function printCouncil(report) {
  ...
  if (missingKeys.length) {
    const noKey = missingKeys.filter(m => !m.undispatchable);
    const undisp = missingKeys.filter(m => m.undispatchable);
    if (noKey.length) {
      console.log(`\nSkipped (missing env keys):`);
      noKey.forEach(m => console.log(`  - ${m.model} (needs ${m.missingEnvVar})`));
    }
    if (undisp.length) {
      console.log(`\n⚠️  Skipped (NOT DISPATCHABLE — no PROVIDERS block; silent-drop guard):`);
      undisp.forEach(m => console.log(`  - ${m.model} — ${m.reason}${m.suggestion ? ` [→ ${m.suggestion}]` : ''}`));
    }
  }
}
