...
// 2026-05-26 — Pattern-based per-gate action composer. Replaces the prior
// generic "Reframe the gate above" copy that couldn't tell the user WHICH
// gate or WHAT to address. Reads the rationale text + matches against known
// gate-type signatures (comp undisclosed, Python primary, customer-facing
// tenure, terminology relabel, pre-apply mitigation) to produce a concrete
// one-sentence move. Falls back to surfacing the rationale itself when the
// pattern is unknown — at minimum the user sees the gate ID + the objection.
function _gateAction(gateId, reason) {
  ...
  if (/comp\s+(?:undisclosed|not\s+disclosed|hidden|withheld)|undisclosed\s+comp/i.test(r)) {
    return `<strong>${esc(gateId)} (comp undisclosed)</strong> — confirm a salary range with the recruiter in the screen call BEFORE submitting. Don't apply blind; the eval capped Comp at 2.5/5 until disclosed.`;
  }
  if (/python\s+(?:primary|production)|primary\s+python/i.test(r)) {
    return `<strong>${esc(gateId)} (Python primary stack)</strong> — frame your agent work (Claude Code, Apps Script, Node.js) as production engineering in the cover letter, and clarify the Python expectation with the recruiter on the screen call.`;
  }
  if (/customer.?facing|enterprise\s+(?:sales|customer)|sales\s+cycle/i.test(r)) {
    return `<strong>${esc(gateId)} (enterprise customer-facing tenure)</strong> — lead the cover letter with your principal-management proof (Ramos, Netanyahu, STIC Summit 348 SEs at 93% CSAT). It's in cv.md — make the recruiter see it before the Python line.`;
  }
  if (/relabel|vector.?database|rag\s+pipeline|terminology/i.test(r)) {
    return `<strong>${esc(gateId)} (terminology)</strong> — relabel "Voice DNA / RAG pipeline" as "context-engineering pipeline with curated corpus + negative-training set" in the tailored CV before submitting.`;
  }
  if (/pre.?apply|mitigation/i.test(r)) {
    return `<strong>${esc(gateId)}</strong> — run /pre-apply-check on this row before submitting. The eval was scored before mitigations were applied.`;
  }
  // Unknown gate type — surface the rationale + the move. Truncate long
  // rationales so the action stays scannable.
