...
import { existsSync, mkdirSync, renameSync, readdirSync, statSync, unlinkSync, rmSync } from 'node:fs';
...
/** Recursively delete iCloud conflict copies inside a .git directory. CAUTION:
 *  this deletes, not archives — the targets are pure filesystem litter.
 *
 *  macOS "Desktop & Documents Folders" iCloud sync makes "<name> 2" /
 *  "index 40"-style duplicates of frequently rewritten git internals (index,
 *  config, refs, logs, worktree indexes). Accumulated copies eventually break
 *  `git fetch` with "bad object refs/heads/<branch> 2". No legitimate git
 *  internal filename OR ref name contains a space, so a trailing " <digits>"
 *  suffix is an unambiguous conflict copy — a real file is never matched.
 *  Directories can themselves be conflict copies (e.g. "hooks 2") and are
 *  removed recursively. objects/ + lfs/ are skipped at the top level
 *  (content-addressed / large, effectively never conflict-copied).
 *  See CLAUDE.md "empty-tree add + iCloud dup storm" bug class.
 *
 *  Returns { removed: [absolute paths deleted] }. */
export function sweepGitIcloudJunk(gitDir) {
  const removed = [];
  if (!existsSync(gitDir)) return { removed };
  const ICLOUD_COPY = / \d+$/;               // space + trailing digits
  const SKIP_TOP = new Set(['objects', 'lfs']);
  const walk = (dir, depth) => {
    if (depth > 10) return;                   // defensive recursion bound
    let entries;
    try { entries = readdirSync(dir, { withFileTypes: true }); } catch { return; }
    for (const e of entries) {
      const full = join(dir, e.name);
      if (ICLOUD_COPY.test(e.name)) {
        try {
          if (e.isDirectory()) rmSync(full, { recursive: true, force: true });
          else unlinkSync(full);
          removed.push(full);
        } catch { /* best-effort; a locked/racing copy is retried next run */ }
        continue;                             // don't descend into a doomed dir
      }
      if (e.isDirectory()) {
