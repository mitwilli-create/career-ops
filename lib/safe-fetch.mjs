 ...
 *
 * Solution: wrap body reads in Promise.race against an independent timer that
 * rejects with a clear error, plus a BEST-EFFORT `r.body?.cancel?.()` (the
 * reader is usually locked by .json()/.text() at that point, in which case
 * cancel() throws and the stalled read is simply abandoned — the caller has
 * already received the timeout rejection either way).
 *
...
function withBodyTimeout(bodyPromise, response, timeoutMs, label) {
  ...
  // up as an unhandled rejection after the race timeout fires.
  bodyPromise.catch(() => { /* race already settled */ });
  let timer;
  const timeoutPromise = new Promise((_, reject) => {
    timer = setTimeout(() => {
      // Best-effort teardown of the stalled stream. The reader is normally
      // locked by .json()/.text()/.arrayBuffer(), so cancel() throws — but
      // when it IS cancellable this releases the socket instead of leaving
      // the read running in the background until the connection dies.
      try { response?.body?.cancel?.(); } catch { /* reader locked — abandoned */ }
