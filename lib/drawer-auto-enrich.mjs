 ...
 *   team-health          → deferred until PR-04 (returns {state: 'deferred'})
...
function slotDispatchCommand(rowId, slot) {
  if (slot === 'hm-intel') {
    return {
      cmd: 'node',
      args: [join(REPO_ROOT, 'scripts/agents/intel-refresh.mjs'), '--slots', 'hm-intel', '--rows', String(rowId)],
    };
  }
  ...
  if (slot === 'team-health') {
    return null; // Deferred — PR-04 lib/team-health-synthesis.mjs
  }
  if (slot === 'context-note') {
    // Degraded-useful path: cv.md sections 1-3 + article-digest first 300 tokens.
    // Until scripts/agents/context-note.mjs ships in PR-08, return a synthetic
    // dispatch that writes a minimal stub to the cache.
    return {
      cmd: 'node',
      args: [
        '-e',
        // Inline inline JS is hard to debug; use a helper script if it ships.
        // For now, no-op + state: 'deferred' is preferable. See PR-08.
        'process.exit(0);',
      ],
      synthetic: true,
    };
