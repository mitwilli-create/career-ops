...
export function renderIndustryGapTable(ranked) {
  ...
  `).join('');
  // Top-of-modal plain-language summary. Surfaces the #1 opportunity +
  // why it's #1 + the next concrete action. The formula text moves to a
  // collapsed "How this is scored" expander below so it's available but
  // doesn't crowd out the takeaway.
  const top = ranked[0] || null;
  const lead = top ? `
    <div style="margin:0 0 14px;padding:12px 14px;background:var(--surface-2);border-left:3px solid var(--green-fg);border-radius:0 6px 6px 0">
      <div style="font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:var(--text-3);font-weight:600;margin-bottom:4px">Top opportunity right now</div>
      <div style="font-size:15px;font-weight:600;color:var(--text);margin-bottom:6px">${escape(top.industry)} <span style="color:var(--green-fg);font-size:13px;margin-left:6px">${top.score}/100</span></div>
      <div style="font-size:12px;color:var(--text-2);line-height:1.5;margin-bottom:8px">${escape(top.notes)}</div>
      <div style="font-size:11px;color:var(--text-3)">
        ${Math.round(top.compensation_density * 100)}% of roles ≥$250K · ${Math.round(top.ai_gap * 100)}% AI-gap · ${Math.round(top.fit_with_mitchell * 100)}% fit with your stack
      </div>
    </div>
    <p style="margin:0 0 8px;color:var(--text-2);font-size:12px">Below: 11 other industries ranked by a composite of <strong>comp density</strong> (% paying ≥$250K), <strong>AI gap</strong> (room for an AI-bridge hire), and <strong>fit</strong> with your comms × builder stack. Click the heading of any column to weigh that signal more heavily.</p>
    <details style="margin:0 0 10px;font-size:11px;color:var(--text-3)">
      <summary style="cursor:pointer;color:var(--text-3)">How this is scored</summary>
      <p style="margin:6px 0 0;padding-left:14px;color:var(--text-3);font-size:11px">Composite 0-100 = 0.40 × comp_density + 0.35 × ai_gap + 0.25 × fit. Edit <code>lib/industry-gap.mjs</code> SEED_TABLE to retune weights or add industries.</p>
    </details>
  ` : '';
  return `
