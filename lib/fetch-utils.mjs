...
// Shared HTTP fetch with a SINGLE end-to-end deadline that covers BOTH the
// fetch() handshake and the body read. AbortSignal.timeout only guards the
// response headers — a stalled body stream can hang past the deadline (see the
// safe-fetch postmortem) — so the body read gets whatever time is LEFT after
// the handshake, not a fresh full budget (which would let a call run up to ~2×
// timeoutMs). Returns { ok, status, text, finalUrl, headers } so callers can
// branch without reimplementing the AbortController/timer dance.
export async function fetchWithTimeout(url, opts = {}, timeoutMs = 15_000) {
  ...
  // Remaining budget for the body read (floor 250ms so a slow-but-live body
  // isn't cut off the instant the handshake ate the budget).
  const remaining = Math.max(250, timeoutMs - (performance.now() - started));
  const text = await readText(res, remaining, url);
  return {
    ok: res.ok,
    status: res.status,
    text,
