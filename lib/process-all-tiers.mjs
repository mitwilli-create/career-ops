...
export const TIERS = {
  1: {
    ...
    // 2026-05-27 — Auto-escalation removed from orchestrator. Polish + pregen
    // are manual-trigger only via row drawer. Flags retained at false for
    // backward compat with any downstream consumer that reads them.
    auto_pregen_on_high_score: false,
    auto_polish_on_high_score: false,
  },
  2: {
    id: 2,
    name: 'Premium Triage',
...
export function tierCostEstimates({
  ...
  //   polish:             $12 per pack  (matches COST_PER_POLISH_PACK_USD env default,
  //                                      post-bugfix-8e83ffa; was $60 from 1000x-inflated traces)
  //
  ...
  // 2026-05-27 — Pregen + polish removed from orchestrator. Tier total now =
  // triage + eval only. Manual pregen/polish surfaces (row drawer + /api/polish
  // + /api/build-pack-stage) carry their own per-row spend that is NOT included
  // in the Process All cost — those are reported in the modal's "Post-publish
  // opt-in" section separately.
  ...
  for (const tid of [1, 2, 3]) {
    ...
    const total = triageCost + evalCost;
    out[tid] = {
      tier: tid,
      name: t.name,
      breakdown: {
