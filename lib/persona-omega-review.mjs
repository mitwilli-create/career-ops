 ...
 * a summary {reviewedCount, findingsCount, costUsd, errors}. When the review
 * does not run (kill switch / no recommendations), the numeric fields are
 * null and the summary carries {unavailable: true, skipped: <reason>}.
 *
...
export async function annotateOmegaRecommendationsWithPersonaReview(recommendations, opts = {}) {
  // No-review paths return null counts + unavailable:true — a 0 here would
  // read downstream as "review ran and found nothing" rather than "review
  // never ran" (silent-zero bug class; data-truth-auditor sweep 4).
  if (process.env.PERSONA_OMEGA_REVIEW === 'false') {
    return { reviewedCount: null, findingsCount: null, costUsd: null, errors: [], unavailable: true, skipped: 'PERSONA_OMEGA_REVIEW=false' };
  }
  if (!Array.isArray(recommendations) || recommendations.length === 0) {
    return { reviewedCount: null, findingsCount: null, costUsd: null, errors: [], unavailable: true, skipped: 'no-recommendations' };
  }
