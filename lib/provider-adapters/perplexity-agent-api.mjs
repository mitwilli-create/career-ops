...
export async function refresh(cache, row, opts = {}) {
  ...
  const userPrompt = opts.promptBuilder ? opts.promptBuilder(cache, row) : defaultPrompt(cache, row, opts.schemaHint);
  ...
  // Truncated output is a writer failure even when safeParseJson salvages a
  // parseable prefix — writing a silently-truncated record poisons the cache
  // and downstream verifiers. 2026-07-08 incident: max_tokens-truncated
  // role_enrichment output surfaced as WRITER_FAILED with an EMPTY error
  // string because ok:!!parsed carried no errors[]. Both failure shapes now
  // return explicit diagnostics (bug class:
  // findings-exit-code-conflated-with-spawn-failure — silent failure signal
  // without the finding).
  if (finishReason === 'length') {
    return {
      ok: false,
      errors: [
        `Perplexity output TRUNCATED at max_tokens=${body.max_tokens} (finish_reason=length, ` +
        `output_tokens=${outputTokens}, raw_content_length=${rawContent.length}). ` +
        `Raise the cache's maxTokens in lib/refresh-cache-registry.mjs.`,
      ],
      contentJson: parsed,
      think,
      costUsd,
  ...
  if (!parsed) {
    ...
    return {
      ok: false,
      errors: [
        `Perplexity output not parseable as JSON (finish_reason=${finishReason || 'unknown'}, ` +
        `raw_content_length=${rawContent.length}). Tail: "...${tail}"`,
      ],
      think,
      costUsd,
      providerMetadata,
...
function defaultPrompt(cache, row, schemaHint = null) {
  // 2026-07-08: without a concrete schema the writer invents a different
  // structure every run, and the cross-architecture verifier correctly
  // rejects the drift against the prior cache. When the orchestrator passes
  // a schema hint (skeleton derived from the prior cached version), pin the
  // output to it explicitly.
  const schemaBlock = schemaHint
    ? `\n\nYour JSON output MUST use EXACTLY this top-level structure (populate every key; use null for unknowns — do NOT drop keys):\n${schemaHint}`
    : `\nReturn strict JSON matching the cache's documented schema.`;
  return `Refresh cache "${cache.id}" for row #${row.num || '?'} (${row.company} — ${row.role}).${schemaBlock}\nEvery fact MUST carry a source URL.`;
}
