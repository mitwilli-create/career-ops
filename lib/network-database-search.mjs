...
function applyFilters(p, filters) {
  ...
  // BRAVO followup 2026-05-20 Item 1 — filter chip support.
  // `warm_apply_now`: person has a warm path to ANY apply-now-queue company.
  //   The warm_to_target_companies entries already encode the universe of
  //   targets we track, so any entry with `apply_now: true` qualifies.
  //   (If absent we fall through to permissive: warm_to_target_companies > 0.)
  if (filters.warm_apply_now === 'true' || filters.warm_apply_now === true) {
    const w = p.warm_to_target_companies || [];
    if (!w.length) return false;
    const hasFlag = w.some(t => t.apply_now === true);
    if (!hasFlag) {
      // Permissive fallback: a warm path to ANY target counts as
      // "warm to apply-now" until the upstream pulse run starts
      // tagging entries with `apply_now: true`. This keeps the chip
      // useful from day 1.
      if (!w.length) return false;
    }
  }
  // `warm_path_min`: any warm path with strength ≥ N.
  if (filters.warm_path_min) {
    const min = Number(filters.warm_path_min);
    const w = p.warm_to_target_companies || [];
    const ok = w.some(t => Number(t.strength || t.warm_path_strength || 0) >= min);
    if (!ok) return false;
  }
  return true;
}
