...
export async function polishArtifact(input) {
  ...
  // Heartbeat emission — matches apply-pack-polish.mjs's emitProgress() pattern
  // (NDJSON to stderr, dashboard SSE forwards to clients). Pattern 7 of
  // 2026-05-19 hang-pattern audit: a long-running orchestrator MUST emit at
  // least one event per step so external watchers can distinguish progressing
  // from hung. Fires at the start of every round, every adjudicator step,
  // and every adversarial sweep.
